...
// 桌面端（≥1024px）双列卡片网格，茧房外栏通栏；移动端单栏

function isCocoon(column) {
  return (column || '').includes('茧房外');
}

function DailyItem({ it }) {
  return (
    <a
      href={it.url}
      target="_blank"
      rel="noopener noreferrer"
      className="flex gap-3 px-4 py-3 border-t t-border transition-colors hover:bg-[var(--surface-2)]"
    >
      <div className="flex-1 min-w-0">
        <h3 className="text-[14.5px] font-medium leading-snug t-text">{it.title}</h3>
        {it.summary && (
        ...
        <div className="mt-1.5 flex items-center gap-2 text-[11px] t-muted">
          ...
          {Array.isArray(it.related) && it.related.length > 0 && (
            <span className="flex-none badge t-accent">+{it.related.length} 相关</span>
          )}
        </div>
      </div>
      <Cover src={it.cover} className="flex-none w-[72px] h-[54px] rounded-lg object-cover" />
    </a>
  );
}
...
export default function DailyTab() {
  ...
  if (failed) {
    return <div className="card mx-4 mt-4 px-4 py-16 text-center text-[13px] t-muted">日报数据加载失败</div>;
  }
  if (!daily) return <div className="py-20 text-center text-[13px] t-muted">加载中…</div>;
      ...
      <div className="mt-3 grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
        {sections.map((sec) => {
          ...
          return (
            <section
              key={sec.column}
              className={`card overflow-hidden ${cocoon ? 'lg:col-span-2' : ''}`}
            ...
            >
              ...
              <div>
                {sec.items.map((it, i) => (
                  <DailyItem key={it.ref_id ?? i} it={it} />
                ))}
              </div>
            </section>
          );
