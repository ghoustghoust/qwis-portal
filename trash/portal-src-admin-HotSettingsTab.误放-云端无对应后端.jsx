import { useCallback, useEffect, useState } from 'react';
import { api } from '../api';
import { toast } from '../toast';

// 热点榜设置 Tab（管理后台独立顶层导航）：AIHOT 专属刷新间隔 + 分类规则表（只读）+ 热榜页启用开关
const DEFAULT_MAP = {
  模型: ['模型发布', '评测/基准'],
  产品: ['产品更新'],
  行业: ['行业动态'],
  论文: ['论文'],
  教程: ['教程/实践'],
  观点: ['大佬观点', '现象/趋势'],
};

function isAggregator(s) {
  if (/aihot/i.test(s?.name || '') || /aihot/i.test(s?.url || '')) return true;
  try {
    const extra = typeof s?.extra === 'string' ? JSON.parse(s.extra) : s?.extra;
    return !!extra?.aggregator;
  } catch {
    return false;
  }
}

export default function HotSettingsTab({ toast: parentToast }) {
  const [aihot, setAihot] = useState(null);
  const [catMap, setCatMap] = useState(DEFAULT_MAP);
  const [enabled, setEnabled] = useState(true);
  const [intervalMin, setIntervalMin] = useState('');
  const [busy, setBusy] = useState(false);
  const [backfill, setBackfill] = useState(null); // {running,total,done,failed}
  const [backfillReady, setBackfillReady] = useState(true);

  const loadAihot = useCallback(async () => {
    try {
      const data = await api.get('/api/sources?type=rss');
      const items = Array.isArray(data) ? data : data?.items || data?.sources || [];
      const found = items.find(isAggregator);
      setAihot(found || null);
      if (found && found.intervalMin) {
        setIntervalMin(String(found.intervalMin));
      }
    } catch (e) {
      setAihot(null);
      setIntervalMin('');
    }
  }, []);

  const loadSettings = useCallback(async () => {
    try {
      const hotConfig = await api.get('/api/settings/hot');
      setEnabled(hotConfig?.enabled !== false);
    } catch {
      setEnabled(true);
    }
  }, []);

  useEffect(() => {
    loadAihot();
    loadSettings();
    api
      .get('/api/hot/categories')
      .then((d) => {
        if (d?.map && Object.keys(d.map).length) setCatMap(d.map);
      })
      .catch(() => {});
  }, [loadAihot, loadSettings]);

  // 回填状态轮询
  useEffect(() => {
    let timer = null;
    let stopped = false;
    const poll = async () => {
      try {
        const d = await api.get('/api/hot/backfill');
        const st = d?.status || d;
        if (stopped) return;
        setBackfillReady(true);
        setBackfill(st);
        if (st?.running) timer = setTimeout(poll, 2000);
      } catch {
        if (!stopped) setBackfillReady(false);
      }
    };
    poll();
    return () => {
      stopped = true;
      if (timer) clearTimeout(timer);
    };
  }, []);

  const startBackfill = async () => {
    if (backfill?.running) return;
    try {
      await api.post('/api/hot/backfill');
      setBackfill((b) => ({ ...(b || {}), running: true, done: b?.done ?? 0, total: b?.total ?? 0, failed: b?.failed ?? 0 }));
      const poll = async () => {
        try {
          const d = await api.get('/api/hot/backfill');
          const st = d?.status || d;
          setBackfill(st);
          if (st?.running) setTimeout(poll, 2000);
          else parentToast(`回填完成：${st?.done ?? 0}/${st?.total ?? 0}${st?.failed ? `，失败 ${st.failed}` : ''}`);
        } catch { /* 忽略 */ }
      };
      setTimeout(poll, 2000);
    } catch (e) {
      parentToast(e.message || '回填启动失败');
    }
  };

  const saveInterval = async () => {
    const n = Number(intervalMin);
    if (!Number.isFinite(n) || n < 1) {
      parentToast('间隔最小 1 分钟');
      return;
    }
    setBusy(true);
    try {
      await api.put(`/api/sources/${aihot.id}/interval`, { intervalMin: n });
      parentToast(`AIHOT 刷新间隔已设为${n}分钟`);
      await loadAihot();
    } catch (e) {
      parentToast(e.message || '保存失败');
    } finally {
      setBusy(false);
    }
  };

  const toggleEnabled = async () => {
    setBusy(true);
    try {
      await api.put('/api/settings/hot', { enabled: !enabled });
      setEnabled(!enabled);
      parentToast(!enabled ? '热点榜已启用' : '热点榜已停用（导航入口已隐藏）');
    } catch (e) {
      parentToast(e.message);
    } finally {
      setBusy(false);
    }
  };

  return (
    <div className="space-y-4">
      {/* 启用开关 */}
      <div className="card p-5">
        <h3 className="text-sm font-semibold t-text mt-0">热点榜页面</h3>
        <p className="mt-1 text-xs t-muted mb-3">
          /hot/ 页面以 AIHOT 为数据源，提供分类精选、全部动态与中英对照阅读。
        </p>
        <label className="flex items-center gap-2 text-[13px] cursor-pointer">
          <input
            type="checkbox"
            checked={enabled}
            onChange={toggleEnabled}
            disabled={busy}
          />
          <span>启用热点榜页面（导航栏 🔥 入口）</span>
        </label>
      </div>

      {/* AIHOT 间隔配置 */}
      <div className="card p-5">
        <h3 className="text-sm font-semibold t-text mt-0">AIHOT 刷新间隔</h3>
        <p className="mt-1 text-xs t-muted">设置 AIHOT 专属的抓取频率（影响所有 newsnow 热榜源）。</p>
        
        <div className="mt-3 flex items-end gap-2">
          <div className="field" style={{ width: 120 }}>
            <label>间隔（分钟）</label>
            <input
              type="number"
              min="1"
              value={intervalMin}
              onChange={(e) => setIntervalMin(e.target.value)}
              disabled={busy || !aihot}
            />
          </div>
          <button
            className="btn"
            onClick={saveInterval}
            disabled={busy || !aihot}
          >
            保存间隔
          </button>
        </div>
        
        {!aihot && (
          <p className="mt-2 text-xs t-muted">
            未找到 AIHOT 源（在订阅源管理中添加 newsnow 类型源后可见）
          </p>
        )}
      </div>

      {/* 历史回填 */}
      <div className="card p-5">
        <h3 className="text-sm font-semibold t-text mt-0">历史内容回填</h3>
        <p className="mt-1 text-xs t-muted">
          从 AIHOT sitemap 抓取尚未入库的历史条目（一次性操作）。
        </p>

        <div className="mt-3">
          <button
            className="btn-ghost"
            onClick={startBackfill}
            disabled={!backfillReady || !!backfill?.running || busy}
          >
            {backfill?.running ? '回填中…' : '开始回填历史'}
          </button>
          {!backfillReady && <span className="text-xs t-muted ml-2">回填接口尚未就绪</span>}
        </div>

        {backfill && (backfill.running || backfill.total > 0) && (
          <div className="mt-3">
            <div className="h-2 rounded-full overflow-hidden" style={{ background: 'var(--surface-2)' }}>
              <div
                className="h-full t-accent-bg transition-all"
                style={{
                  width: `${backfill.total ? Math.min(100, Math.round(((backfill.done || 0) / backfill.total) * 100)) : 0}%`,
                }}
              />
            </div>
            <div className="mt-1 text-xs t-muted tabular-nums">
              {backfill.done ?? 0}/{backfill.total ?? 0}
              {backfill.failed ? `（失败 ${backfill.failed}）` : ''}
              {backfill.running ? ' · 每条约 2s，回填期间不影响正常刷新' : ' · 已完成'}
            </div>
          </div>
        )}
      </div>

      {/* 分类规则表 */}
      <div className="card p-5">
        <h3 className="text-sm font-semibold t-text mt-0">分类归类规则</h3>
        <p className="mt-1 text-xs t-muted">仅展示，不可编辑。未命中的条目将归入「全部」胶囊。</p>

        <div className="mt-3 overflow-x-auto">
          <table className="w-full text-[13px]">
            <thead>
              <tr className="t-surface2 text-left">
                <th className="px-4 py-2 font-medium t-muted text-xs">胶囊分类</th>
                <th className="px-4 py-2 font-medium t-muted text-xs">AIHOT 分类映射</th>
              </tr>
            </thead>
            <tbody>
              {Object.entries(catMap).map(([cat, sources]) => (
                <tr key={cat} className="border-t t-border">
                  <td className="px-4 py-2 t-text">
                    <span
                      className="badge-green"
                      style={{ color: 'var(--accent)', borderColor: 'var(--accent)' }}
                    >
                      {cat}
                    </span>
                  </td>
                  <td className="px-4 py-2 t-muted">
                    {Array.isArray(sources) ? sources.join('、') : String(sources)}
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </div>
    </div>
  );
}
