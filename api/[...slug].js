...
async function handleDaily(req) {
  ...
  if (hour >= 1) {
    try {
      ...
      // T6（09-24 审查抓出的第三处不同形）：这一支是**当天第一次有人打开早报**时走的（库里还没有行 → 现场生成），
      // 原来只回 generated_at/sections/stats，漏掉 theme/schemaVersion/degraded ⇒ 前端在这条最常走的路上看不到导语与档位标记。
      return jsonOk({
        report: {
          generated_at: report.generated_at, sections: await enrichDailyTranslated(report.sections), stats: report.stats,
          theme: report.stats.theme || null,
          schemaVersion: report.stats.schemaVersion || 1,
          degraded: !!report.stats.degraded,
        },
        stale: false, autoGenerated: true,
      });
    } catch (err) {
...
async function handleDailyRegenerate(req) {
  ...
  const report = await generateDailyInline();
  return jsonOk({
    // 同 T6：管理后台"重新生成"后回给前端的那一份也必须带档位/导语，否则点一次按钮就"导语消失"。
    report: {
      generated_at: report.generated_at, sections: report.sections, stats: report.stats,
      theme: report.stats.theme || null,
      schemaVersion: report.stats.schemaVersion || 1,
      degraded: !!report.stats.degraded,
    },
