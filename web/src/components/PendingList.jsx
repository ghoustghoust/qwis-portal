...
const STATUS_MAP = {
  pending: { text: '待处理', color: 'var(--accent)' },
  resolved: { text: '已解析', color: 'var(--green)' },
  failed: { text: '解析失败', color: 'var(--red)' },
};
          ...
          <div key={it.id ?? it.url} className="flex items-center gap-3 px-4 py-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="badge-green" style={{ color: st.color, borderColor: st.color }}>
                  {st.text}
                </span>
              </div>
