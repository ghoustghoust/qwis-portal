...
| `npm run lint:docs` | `tools/doc-lint.cjs`（密钥判据 `lib/secrets.js`） | 文档门禁**十三条**（头注/悬空/INDEX/归档头/超长/**明文密钥＝两面扫描：已跟踪 + 未跟踪且未被 ignore，模式补飞书/钉钉/企微 webhook 与长 Bearer** + 09-21 扩面：**表格超格 / 表格断裂 / 活锚点行号越界 / F2P 条数须带证据文件名 / 同表编号唯一 / 父 spec 背景断言须带出处(提示级) / 判据自证**） | ✅ 0 错 22 警（`docs/eval/` 机器产物不参与悬空扫描；锚点分母每次运行都会打印：本轮看到 159 条活锚点、23 条按死锚点信号放过、越界 0 条） |
