...
| **P2（09-24 登记）** | **源侧治理**（并入后台重构，要细看）：僵尸源自动停用（近 14 天 0 产出；实测启用 1,129 而真出内容仅 761）、非文章流按形态出池（`Recent Commits to openclaw` 现为池内第一大户 39.6 篇/日） | 与后台信息架构一并做 | **注意口径**：级3 之后每源最多占 2 坑，**砍大源换不来覆盖**，收益是存储/调度轮次/翻译额度 |
