...
|---|---|---|---|
| `npm test` | `tests/*.test.js`（node:test，`--test-concurrency=1`） | 回归网：每条线上修过的 bug 都要有锁 | ✅ **640** 条 / **0 红** / 0 跳过（**09-24 18:28Z 复跑读数，EXIT=0**；637→**640** = **T5**（全仓扫每一处 `generateThemeDetailed(` 必须投影 `.theme` 或落归因）+ **T6**（部署面五处 `report` 字面量的顶层键同形）+ 执行锁 **I4**（`file:` 临时库上把读层四条早报分支各跑一遍，断言三个字段都在**且值真从 `stats` 来**）。⚠️ 到这 640 之前先红过 3 条，三红全是我自己造的：**B64-1 第二次中招**（交付探针里又写死了一份云端基址 —— 上一格刚记过同族，这轮再犯 ⇒ 教训是"先 `grep` 唯一实现，再谈写常量"）+ 两条 doc-lint 类（活文档引用了只在本地、git 不会带走的探针）。633→637 = 新文件 `tests/regression-theme-observe.test.js` **T1~T4**（H30：导语归因三态必须可区分 / 空输入也要带 why / `generateTheme` 旧字符串契约不坏 / runner 必须落 `themeSkip`）。⚠️ 同轮一条**真红是自家探针造的**：`tools/_probe-reader-visible-tier.cjs` 手写了一次云端域名 ⇒ **B64-1**（"云端基址全库一份"）当场判红 —— 锁连 gitignore 的探针一起扫，处置是**改代码不是改锁**（改成 import `lib/cloud-site`）。626→633 的 +7 = **P11** 契约 stats 形状手抄清单 + **P12** 契约键集合改为**从四份 `const stats = {…}` 字面量派生**（`tools/stats-literal-keys.cjs`，自带合成坏样本自证）+ 新文件 `tests/regression-prescreen-arms.test.js` **A1~A4**（三臂打分方向锁：CNB 反号 bug 的回归锁 / `>=0` 判留与分数同向 / NB 长度归一不许删 / 脚手架被 require 时不许跑驱动）；上一格 626 已滞后一次，本轮以复跑为准）。历史：P9 宽池读不分叉 + F6 夜间预算三段自洽 +
