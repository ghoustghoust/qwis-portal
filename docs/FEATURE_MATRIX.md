...
| **P1（09-24 夜新立·H32）** | **「主题全景」0 簇的真因不是阈值**：`buildThemePanorama:1064` 是 `if (!r.ok) continue` —— 起名一失败**整簇消失**，读数却记成"聚不到"。离线回放（生产同一份贪心实现 + 漂移守卫）：**@0.45 已有 8 簇分布在 6 期**，而库里 16 期总共只存下 3 簇；放宽到 0.25 变 15 簇但**最大簇始终 2~3 条** | 归因键 + 退化命名 | 三条待拍（接归因／起名失败不许吃整簇／小批次是否可接受空态），**阈值与出口本轮都不动** |
