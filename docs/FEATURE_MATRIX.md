...
| **P1（09-24 新立·H26，09-24 夜改判）** | **数据体量与读放大治理**：**主路不是归档**（90 天窗口只覆盖 7.8% 正文），而是①不常驻 `content_html`／入库前压缩 ②三个 UI 入口的扫描宽度（精选栏 `CAST(score)`、搜索扫正文、早报详情传 20 份） | 见 `docs/ISSUES.md` H29 三条待拍板 | 库里 **97.0% 是一列 `articles.content_html`**（**587.0 MB** / 35,446 行，字节口径 `LENGTH(CAST(x AS BLOB))`）；日增 **99.6 MB 正文**，其中 **98.6% 来自进阅读器的全文源**（腹泻类热榜源被降噪轴排除，4,722 行只带 1.4 MB）——**"砍腹泻源救体积"不成立** |
