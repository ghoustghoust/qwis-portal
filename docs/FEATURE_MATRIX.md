...
| **P0（09-24 查出·H27）** | **AI 早报的自动触发未兑现**：晚间主批 `daily-ai-evening` 近 8 次 schedule **8/8 skipped**，`daily-ai` 1/8；库里 AI 行落库时刻对不上任何 cron 点位，09-19 及以前 AI 版 0 天 → 现在靠人工 dispatch 维持 | 三选一：拆独立 workflow / 外部定时 dispatch / collect 尾部到点自检补跑。<b>⚠️ 09-24 17:10Z 机制改判（H27-补）</b>：新样本显示 `github.event.schedule` 串等值这道门会开在错误的 job 上（06:24Z 那次只跑了 snapshot），所以 (a) 一文件一 cron／(b) 挂 dispatch + 时间窗幂等自检 两条都可行、(b) 才免疫 schedule 丢失。<b>三侧边界与余量对账总表：`docs/eval/2026-09-24-boundaries.md`</b> | 取证与机制候选见 `docs/eval/2026-09-24-source-diagnosis.md` §四。⚠️ 本轮把夜间预算抬到 300min、超时 330min，会让"上次还在跑就丢 schedule"更易发生：**收益与风险一起观察** |
