...
| 热榜/事件榜（全局时间序） | ✅ | ✅ | 2026-09-11 修复同源成块；2026-09-14：「实时流」更名「AI 信息实时流」并只出 AI 相关内容（AI 分组源全收 + 标题词表过滤，词表见 `api/[...slug].js` AI_KW_*）；事件卡带「分组·信源名」胶囊 + 24 桶趋势折线；分类 pills 已接 /api/groups |
