...
> 最后更新：2026-09-23（§1.5 复跑校核：`npm test` 实测 **604** 条 / 0 红（原写 544）；白盒实存 **24 项**，`W5/W8/W21/W22` 从未实现，文档不得再写"W1~W11"或"W9~W21"；§1.5 与保留面两处指向 43/41 号 spec 的引用改为作废指针，锚点见 `docs/ISSUES.md`。上轮 2026-09-20（换库后的矩阵同步：①采集矩阵里「公众号」那一格按实测从 ✅ 改成 **❌ —— 两份云端实现各有 `UNSUPPORTED_TYPES` 且都排除 wemp**，wemp 893 篇最新一篇停在 09-04（B119）；②「早报体系 v2 ✅ 已完成」补 B120 欠账（AI 版条目从 09-13 起就没封面图，已修 + 出证）；③周刊归档改按 09-20 午后读数：新第 1 期已生成但缺主线骨架 = B121）
