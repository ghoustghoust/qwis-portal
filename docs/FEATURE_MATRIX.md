...
|---|---|---|---|
| `npm test` | `tests/*.test.js`（node:test，`--test-concurrency=1`） | 回归网：每条线上修过的 bug 都要有锁 | ✅ **420** 条 / 0 红 / 0 跳过（约 120s；**B83 已收口——七份"直打生产 Turso"的回归测试全部搬到本地文件库**，不再有分钟级缓存轮询，也不再往生产数据层写东西。**本行是全库唯一写死测试条数的地方**，其它文档一律写"见 FEATURE_MATRIX §1.5"） |
...
| `npm run eval:whitebox` | `tools/eval-whitebox.cjs` | 三端一致性 W1~W14（含 W3b 假开关、W9 坑↔锁对账、W10 重复判定、W11 入口 Provider 完整性、W12 媒体栏字段对账、**W13 日报栏目表只许一份**、**W14 入报门槛必须接在每一个 `INSERT INTO daily_reports` 的宿主函数里**） | ✅ 全过（W12/W13 都做过负向验证：摘掉一处 `source_avatar` 即红并点名行号；塞一份探针栏目表即红并点名文件。W14 的负向验证 = 把任一份门槛摘掉即红并点名 `函数@文件`，本轮正是靠它抓出"5 个写入点只接了 3 个"） |
