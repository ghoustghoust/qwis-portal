...
> 最后更新：2026-09-24 夜（**第三/四轮复审收口**：§1.5 条数 637→**640**（T5/T6 + 执行锁 I4）；**H30** 从"94% 的期"重写成三层口径（期 / 北京日 / 读者实得），并补上"归因只接了早报 AI 档，周刊与我的早报仍静默"的范围；读层响应同形补到**第 5 处**（含 `api/daily-generate.js` 的回执）；新立 **H31** —— `/api/articles` 的 `limit` **不是本端点参数**（`?limit=5`/`?limit=50`/不传都回 30 条），两处自家取证脚本一直以为拿的是 3 行。上一格读数：626→633（P11/P12 契约锁 + A1~A4 三臂方向锁）+ §1.5 新增两条 `eval-prescreen-*` 命令行（零额度初筛层效果实验，含 09-24 夜首版"是否采用"答案）+ §2 新立 **H29 删除闸时钟错配** 行、H26 行按字节口径改判。上轮：44 号 spec 步1、`eval:*` 降级为按需工具、push-CI 缺口核销）
...
|---|---|---|---|
| `npm test` | `tests/*.test.js`（node:test，`--test-concurrency=1`） | 回归网：每条线上修过的 bug 都要有锁 | ✅ **643** 条 / **0 红** / 0 跳过（**09-24 21:07Z 复跑读数，EXIT=0**；641→**643** = **T8**（每个 `buildThemePanorama(` 调用点必须取 `.themes` —— 返回形状改成对象后，漏一个调用方的症状正是"主题全景静默变空"）+ **T10**（函数体内每条 `continue` 前必须有 `bump(`；**上岗当天就抓到我自己漏计的第 5 条出口** `drops.no_token`）；640→641 = **T7**（把"注释/字符串里的 `report: {` 不算返回点、真返回点的顶层键要读对"当成提取器自己的坏样本喂进去）+ I4 从四臂扩到**五臂**（补 `POST /api/daily-generate` 回执，并加"错 key 必须 403 且无 report"的反例）。⚠️ 同一轮自己 F2P 出一条**未收口**：I4 的 `consistent` 只对"stats 带非默认值"的两臂有牙，把关键词档那三臂的 `theme` 写死成 `null` 仍然绿 —— 要出证据得让内联生成器产出非默认 stats（要打模型，本用例打死 fetch ⇒ 不做），已写进锁注释与本表。637→640 = **T5**（全仓扫每一处 `generateThemeDetailed(` 必须投影 `.theme` 或落归因）+ **T6**（部署面五处 `report` 字面量的顶层键同形）+ 执行锁 **I4**。⚠️ 到这 640 之前先红过 3 条，三红全是我自己造的：**B64-1 第二次中招**（交付探针里又写死了一份云端基址 —— 上一格刚记过同族，这轮再犯 ⇒ 教训是"先 `grep` 唯一实现，再谈写常量"）+ 两条 doc-lint 类（活文档引用了只在本地、git 不会带走的探针）。633→637 = 新文件 `tests/regression-theme-observe.test.js` **T1~T4**（H30：导语归因三态必须可区分 / 空输入也要带 why / `generateTheme` 旧字符串契约不坏 / runner 必须落 `themeSkip`）。⚠️ 同轮一条**真红是自家探针造的**：`tools/_probe-reader-visible-tier.cjs` 手写了一次云端域名 ⇒ **B64-1**（"云端基址全库一份"）当场判红 —— 锁连 gitignore 的探针一起扫，处置是**改代码不是改锁**（改成 import `lib/cloud-site`）。626→633 的 +7 = **P11** 契约 stats 形状手抄清单 + **P12** 契约键集合改为**从四份 `const stats = {…}` 字面量派生**（`tools/stats-literal-keys.cjs`，自带合成坏样本自证）+ 新文件 `tests/regression-prescreen-arms.test.js` **A1~A4**（三臂打分方向锁：CNB 反号 bug 的回归锁 / `>=0` 判留与分数同向 / NB 长度归一不许删 / 脚手架被 require 时不许跑驱动）；上一格 626 已滞后一次，本轮以复跑为准）。历史：P9 宽池读不分叉 + F6 夜间预算三段自洽 +
...
| **P0（09-24 夜查出·H30）** | **早报的「主题导语 / 主题全景」长期没产出**：库里 AI 档 16 期 → **theme 缺失 15 期（94%）**、`themes` 主题全景 **14/16 期 0 簇**，而三条失败出口（模型没答 / 每行被污染判据否决 / 挑出的那句被否决）**都不留痕** ⇒ 矩阵挂着 ✅ 而读者侧长期缺功能 | 本轮只补"看得见"：`generateThemeDetailed` + 纯函数 `pickThemeReply`，theme 为空时落 **`stats.themeSkip={why,err?,detail?,lines?}`**，契约收录，锁 **T1~T4** 钉三态可区分 + 旧字符串契约不坏。**下一刀等读数**：跑一期 AI 批看 `why` 是 `ai_failed` 还是 `all_lines_rejected`，再决定动阈值还是动模型/配额；在那之前**不许先放宽判据** | 只读探针 `tools/_probe-ai-feature-presence.cjs`（数出来 15/16 空）；同轮附带记录：新派生锁 P12 第一版**看不见条件展开** `...(c ? {} : { themeSkip: … })`，被它自己的坏样本自证抓出后修好（圆括号当透明、只取分支对象第一层） |
| **P0→P1（09-24 查出·H27，同夜 20:12Z 按点位重算后降级并改判）** | ~~**AI 早报的自动触发未兑现**：晚间主批 `daily-ai-evening` 近 8 次 schedule **8/8 skipped**，`daily-ai` 1/8；库里 AI 行落库时刻对不上任何 cron 点位，09-19 及以前 AI 版 0 天 → 现在靠人工 dispatch 维持~~ ⇒ **这句被第五轮审查判错、我复跑确认**：按真实点位算，schedule 侧 09-12 起 `daily-ai`+`daily-ai-evening` 共 **23 次 success**，两根每日 cron 自 09-13 存在（应有 ~24 根）⇒ **触发率 96%，AI 批基本天天在跑、天天在写库**；"靠人工 dispatch 维持"不成立。**仍真的两件事**：① 偶发**归属开错 job**（`run 35964359306` 06:24Z 那次只跑了 snapshot，铁证未被推翻）；② **15 分钟增量线实测投递 ≈7%**（175 次 success ÷ 25.8 天 ≈ 6.8 次/日，应 96 次/日；比先前写的 12% 更差），每日单根线（daily-report/snapshot/cleanup）≈50%。⇒ 本行从 P0 降 P1，修法优先级重排：先治**增量线丢 93%**，别再去给已经 96% 的两根每日线加触发口 | 三选一仍在（拆独立 workflow / 外部定时 dispatch / collect 尾部到点自检补跑），但**用途改小了**：只对付"偶发错版 + 每日线 50%"。<b>⚠️ 09-24 17:10Z 机制改判（H27-补）</b>：`github.event.schedule` 串等值这道门会开在错误的 job 上（06:24Z 那次只跑了 snapshot），所以 (a) 一文件一 cron／(b) 挂 dispatch + 时间窗幂等自检 两条都可行、(b) 才免疫 schedule 丢失。<b>三侧边界与余量对账总表：`docs/eval/2026-09-24-boundaries.md`</b> | 取证与机制候选见 `docs/eval/2026-09-24-source-diagnosis.md` §四。⚠️ 本轮把夜间预算抬到 300min、超时 330min，会让"上次还在跑就丢 schedule"更易发生：**收益与风险一起观察** |
| **P1（09-24 夜新立·H31）** | **`/api/articles` 没有 `limit` 这个参数，且不报错**：线上实测 `?limit=5`／`?limit=50&page=2`／不传**都回 30 条**；`api/[...slug].js:126` 与 `server/routes/articles.js:9` 各写死一份 `PAGE_SIZE=30`，分页只有 `cursor`。已经咬到取证：两处脚本传 `limit=3` 实际各拉 30 行（在"行读决定成本"的这轮里是隐形配额支出） | 无（一处端点） | **待拍板**：(a) 真支持 `limit`（钳位 + 回显生效值）／(b) 维持 cursor 唯一分页，契约显式写"没有 limit"并改掉误导脚本 |
| **P1（09-24 夜新立·H32）** | **「主题全景」0 簇：病根不在阈值，而簇进库前要连过**四条**不留痕出口**（`!r.ok`／匹配不到 `{…}`／`!name` 或 `!summary` 缺一项／`JSON.parse` 抛）⇒ 任一环节丢就整簇消失，读数却记成"聚不到"。离线回放（生产同一份贪心实现 + 函数文本漂移守卫）：**@0.45 至少 8 簇分布在 6 期**，而库里 16 期总共只存下 3 簇 —— ⚠️ 这是**下界**：回放只喂成栏的 46 条，线上 `:1255` 喂的是 `analyzed` 的 137~215 条。放宽到 0.25 变 15 簇但**最大簇始终 2~3 条**（换来的是一条条两条一组的近似重复） | 四条出口各接计数 + 退化命名 → **① 已做（09-24 21:00Z）**：`stats.themePanorama={found,multi,named,drops}`，判定一字未动；锁 T8/T10（T10 上岗即抓到我漏计的第 5 条出口 `drops.no_token`） | **具体哪条在丢从此可查**；剩 ② 起名失败不许吃整簇、③ 小批次空态是否可接受 等你拍（阈值与出口都不动） |
...
| **P1（09-24 新立·H26，09-24 夜按杠杆排序改判）** | **数据体量与读放大治理**：**体积侧最大的一刀是正文自压**（gzip-9 实测 5.90× ⇒ 587 MB→≈100 MB，省 83%，不动源不删行），其次是放行清理（235 MB / 41.6%）；**"截断 200 KB/篇"只值 13.6%、"砍腹泻源"在体积上只值 1.6%**。读放大侧：三个 UI 入口各按实测口径改（见 H29 行） | 压缩属跨端改造（写三份 + 读多处 + `content_html LIKE` 必须换实现）⇒ 按 spec 走，本轮只登记杠杆表 | 存量按轴实测：可见 98.3% / 排除 1.7%（行数前 12 的源占 22.9% 行、1.6% 正文；AIHOT 热榜与精选全文=排除、日报=可见）；单篇分布 <20 KB 占 82.1% 行 / 11.8% 字节，>300 KB 仅 193 行。`docs/eval/2026-09-24-turso-read-amp.md` §九 |
| **P0（09-24 夜查出·H29，同夜按对抗审查改写）** | **删除闸把"可删的东西"永久锁在库里**：23h 心跳里 cleanup 只出现 **1 次**，那一次 `deleted:0 / blocked:"delete-gate" / 待删 8,525`；待删已堆到 **9,472 行 / 正文 235.2 MB（全表 41.6%）**，外加 453 MB 空闲页。**根因是时钟错配**：行要满 **7 天**才可删，凭证只在人工 dump 后 **48h** 内有效 —— 闸开着时几乎没得删，等有得删时闸已关，且**全程无报警** | ① 两条报警（挡住 >24h ＋ 当日无 cleanup 心跳）② 续期路径二选一 ③ 三个 UI 扫描入口（口径见"实测"格） | 逐日读数 `settings['retention.pending']` + 执行证据 `settings['cloud.collect'].history`；⚠️ 配额维度**不可观测**（无平台 token），"被禁的是读"推不出"先撞行读"；`candidates` 在 AI 档位与关键词三份**口径不同**（前者＝配额后送模型行数），跨档比较必错。全量：`docs/eval/2026-09-24-turso-read-amp.md` |
| **P1（09-24 新立 → 同日 15:35Z 已出首版答案）** | **零额度初筛层效果实验**：朴素贝叶斯 / 互补 NB / TF-IDF 质心 vs 现 AI 初筛，**主判据＝误砍率**（用户标"留"却被砍），次判据＝漏砍率 | 结论已给（见"实测"格）；**人工那 53 行仍值得勾**——它能把"你读过 vs 没读过"换成"你想不想读"，是收紧工作点的唯一途径 | 用**行为标签**先于人工标注做完三臂回测（正＝已读 246/稍后读 3/精选 777，负＝保留策略判定可删的抽样 3,033；训练侧完全不含你读过的那批）：AUC **.66~.71**[.63,.75]，与长度单特征对照 .48[.43,.52]、随机 .50 **区间不重叠**⇒信号是真的、不是"穿统计外衣的长度规则"。**但**约束放在误砍≤3% 时**最多只能砍 10%**（≤5%→15%），砍 50% 会连带干掉你读过的 **26~28%** ⇒ **采用形态＝形态规则之后的"底部一小段"兜底排序，不是整段替代 AI 初筛**（那 67~98min 省不掉）；三臂互相不可分，选模型是伪问题。脚手架顺手抓出一个**反号 bug**（互补 NB 的 argmin 应为 argmax，AUC 从 0.082 修到 0.918）——原「NB 家族不可信」那句据此改成只成立一半。命令 `node tools/eval-prescreen-behavior.cjs`（零网络零模型 4s），读数 `docs/eval/2026-09-24-prescreen-labels.md`「三臂行为回测」 |
...
| **P2（09-24 登记）** | **「运维键 vs 人可配键」显式分类**（H28）：`ai.minIntervalMs`／`ai.filterThreshold`／`mybrief.interestProfile`／`translate`／`reading.digest`／`hot.categories` 部署面零写点；另有 7 个是机器自写状态键（不该做格子）、5 个已有专门端点（别误报） | `ai` 段 env-only 裁决要不要为"调参"开例外（09-11 事故裁决的续篇） | 清单与判据 `docs/eval/2026-09-24-source-diagnosis.md` §六。**直接卡住当前的 gap 议题**：4s 间隔在云端改不动 |
| **P2（09-24 已做代码，待你目测）** | 早报设置页「每源每日配额」一格 + `GET /api/settings` 透出归一值 + 锁 P10 四头对齐；`PUT` 写路径同轮补上 | 无 | **未做：浏览器里登录后目测**（点保存属生产写，未授权）→ 现状是"构建通过 + 锁 + 读侧真回值"，不等于你已经在页面上看见 |
