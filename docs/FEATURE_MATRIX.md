...
| **P1（09-24 夜新立·H32）** | **「主题全景」0 簇：病根不在阈值，而簇进库前要连过**四条**不留痕出口**（`!r.ok`／匹配不到 `{…}`／`!name||!summary`／`JSON.parse` 抛）⇒ 任一环节丢就整簇消失，读数却记成"聚不到"。离线回放（生产同一份贪心实现 + 函数文本漂移守卫）：**@0.45 至少 8 簇分布在 6 期**，而库里 16 期总共只存下 3 簇 —— ⚠️ 这是**下界**：回放只喂成栏的 46 条，线上 `:1255` 喂的是 `analyzed` 的 137~215 条。放宽到 0.25 变 15 簇但**最大簇始终 2~3 条**（换来的是一条条两条一组的近似重复） | 四条出口各接计数 + 退化命名 | **具体哪条在丢本轮没测出来** ⇒ 不许指派主因；三条待拍见 ISSUES H32，阈值与出口都不动 |
