...
| **P0（09-24 夜查出·H29）** | **删除闸恒挡 → 库"只涨不删"，且无人出声**：`cleanup` 每天跑、`闸=false`，待删堆到 **9,472 行 / 235.2 MB 正文（全表 41.6%）**；上游原因是安全阀的"转储凭证"TTL 48h 而续期是**本地人工动作**、runner 那条磁盘腿结构性永不成立 | ① 报警接"闸挡住 >24h"（纯增益）② 续期路径二选一 ③ 三个 UI 读放大各自可回退 | 逐日读数 `settings['retention.pending']`（09-21 true → 09-23 起 false）；Turso 三维度 rows read/written/storage，09-20 事故文案是被禁**读** → 表越大、一次全扫的行读单价越高（现役 35,446 行 ⇒ 免费档约 14,100 次全扫/月）。全量：`docs/eval/2026-09-24-turso-read-amp.md` |
| **P1（09-24 新立）** | **零额度初筛层效果实验**：朴素贝叶斯 / TF-IDF 质心 vs 现 AI 初筛，**主判据＝误砍率**（用户标"留"却被砍），次判据＝漏砍率 | 脚手架已就位 `tools/eval-prescreen-learning.cjs`（三臂 LOO + 分歧带，零网络零模型）；**只差标注** `docs/eval/2026-09-24-prescreen-labels.md`（53 条） | 合成自测已量出一个前提：**几十条标注的规模下 NB 家族不可信**（多项式 NB 全判砍、互补 NB 全判留；TF-IDF 质心两向都有错）→ 先按 TF-IDF 那一臂判，NB 要等标签上到几百条。若误砍率可接受 → 初筛段 67~98min + 500 次免费池调用整段消失；不可接受 → 改特征或放弃，**不硬上** |
