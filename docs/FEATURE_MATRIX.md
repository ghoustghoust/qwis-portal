...
| `npm run eval:whitebox` | `tools/eval-whitebox.cjs`（派生实现 `lib/daily-writers.js` + `lib/src-spans.js`） | 三端一致性 W1~W15（含 W3b 假开关、W9 坑↔锁对账、W10 重复判定、W11 入口 Provider 完整性、W12 媒体栏字段对账、**W13 日报栏目表只许一份**、**W14 入报门槛必须接在每一个 `INSERT INTO daily_reports` 的宿主函数里**、**W15 每个 `MAX(last_fetched_at)` 必须 NULLIF 掉字面串 `'null'`**——两条都是全仓扫真实调用点、函数级/表达式级核对、注释不算代码、清单不靠人记） | ✅ 全过（负向验证逐条做过：W12 摘一处 `source_avatar` 即红并点名行号；W13 塞一份探针栏目表即红并点名文件；**W14 = 5 处写入点 × 3 种摘法（删调用 / 扔掉返回值 / 只留行尾注释）15 例全红且点名 `函数@文件`，复原后绿**，跑法 `node tools/_probe-w14-selftest.cjs`；**W15 上线当轮就自己抓出第 5 处漏网**：`tools/_diag-media.cjs:17`；剥注释的自检 `node tools/_probe-strip-selftest.cjs` → 212 个源文件剥完 0 个语法被破坏） |
