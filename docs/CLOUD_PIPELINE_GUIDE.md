...
> 最后更新：2026-09-19（夜·图下加注「04:13 清理当前不生效」的实测依据 + 用 `cloud.collect.history[].mode` 判触发真假的口径，见 B101/37-5；09-18 文档清洁轮：测试基线不再写死数字；调度图 09-14 已对齐 collect.yml 的**表达式**——表达式真、触发未成立）
