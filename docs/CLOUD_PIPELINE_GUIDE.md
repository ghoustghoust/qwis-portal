...
> 最后更新：2026-09-20（新增**不变量 19**：Turso 是链路单点且**配额是墙钟事件** —— API 层没有任何降级路径、快照只是产物级，故"本轮零部署"不是"不做云端活体探测"的理由；旧库不许删；换存储要三处凭据同步 + 复验 runner 真写进新库。来源 B118/B119，排查步骤见 `docs/RUNBOOK.md` §10.9）
