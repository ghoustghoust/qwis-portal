> 📦 本文件是 docs/ISSUES.md 的 2026-09-13 存档：所列 P0-1~P0-11 / P1 / P2 / P3 及专项分析均已修复或失效核销，仅作历史检索用。当前活跃问题见 docs/ISSUES.md。
...
| P2-10 | ~~taskQueue 重试语义 3 项回归测试失败~~ **已解决 2026-09-12**：定性为**实现退化**（两个真 bug）：① fetchRetryable 拿毫秒数字比较 ISO 字符串列（SQLite 类型序 INTEGER<TEXT 恒假）→ 失败任务永不重试；② WHERE 预过滤 attempts<retries → dead 标记分支不可达。修复=实现侧（completed_at 按 ISO 字符串比较 + dead 判定移入循环），测试仅对齐返回值对象化语义 | 已修复 | retryFailed 恢复重试+dead 语义；npm test 249/249 全绿 | ✅ |
...
| P2-8 | **Vercel 端缺失路由汇总（2026-09-11 逐项核销）**：已移植 ✅ `/api/sources/restore-all`、`/api/health/*`、`/api/queue/*`、`/api/opml/sync`、`/api/rss/refresh`、`/api/backup/*`、`/api/data/*`（snapshot/restore 501）、`/api/audit`、`/api/alerts/*`（只读）、`/api/sources/library`；2026-09-11 再核销：`PUT /api/settings` + `GET/PUT /api/settings/daily`（13-settings-write，10 项回归测试+线上实测通过）✅；仍缺：`sources/batch`、`sources/autoclassify`、`groups` 写、`sources` 写（POST/DELETE/refresh）、`ai/translate/*` | 剩余缺失项对应前端功能在 Vercel 上仍不可用 | `api/[...slug].js` dispatch 表 | 🟡 部分补齐，余项待开发（完整清单见 docs/FEATURE_MATRIX.md §2） |
