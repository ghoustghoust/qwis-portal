...
│                                                              │
│  ┌─────────────┐    ┌──────────────┐    ┌────────────────┐  │
│  │  GH Actions  │    │  Vercel 读层  │    │  Turso (东京)   │  │
│  │  runner 直采 │───►│  + 管理台     │◄──►│  唯一云数据源   │  │
│  │  每 30min    │    │  (api/)      │    │                │  │
│  └─────────────┘    └──────────────┘    └────────────────┘  │
└──────────────────────────────────────────────────────────────┘
```

**核心决策**（2026-09-11 方案A）：**Vercel 为读层主部署 + 管理台**；**采集/日报/清理主链路在 GH Actions runner**（`tools/collect-turso.js` 直写 Turso，每 30min），不再经 Vercel 函数（Hobby 10s 死局）；本地 Express 为开发/灾备用途（抖音 Playwright 仅本地）。
...
| 调度 | node-cron / 自定义 due 驱动 tick 调度器（本地）；GH Actions runner 直写 Turso（云端） |
| 云端 | Vercel Serverless (读层) / Turso (libSQL) / GH Actions runner (采集) / PHP 队列 |
| 部署 | Vercel + GH Actions (生产) / PM2 (本地 fork 单实例) / Nginx 反代 |
