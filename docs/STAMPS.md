...
| docs/archive/debugging/2026-09-21-issues-round2-closed.md | — | 不要求头注 | `b0ac09f` 2026-09-22 12:08 feat(B86/P3-3)+test(BM1~BM3): 本地日报补「视频与播客」栏(与云端同栏同形状,播客走 audioCoverSql 唯一口径); 修初版错引 collectCandidates 内部 cutoff 的作用域错误(静默 catch 会吃掉=假接线,BM1 的栏存在断言挡住)<br>`40d98e0` 2026-09-22 11:45 feat(B8/P2-2)+test(MB1~MB4): 块级排版——mdBlocks 切分器(标题/无序/有序/空行分段)+MdRich 组件(块内仍走 MdText 内联解析,纯元素树无注入面),周刊/日报/热点详情长文位换用; DailyPage 外层 p→div(合法嵌套) |
