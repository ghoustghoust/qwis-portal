...
**统计**：底层文档 一致 8 · 待判 10 · 头部超前 0 · 缺头注 0　|　不要求头注 194 份　|　工作区未提交 5 份　|　总计 212 份
...
| ARCHITECTURE.md | 2026-09-23 | 待判 ⚠ | `b1e3f71` 2026-09-24 15:59 docs(级3 第一期生产读数到手·id=68): 两处原预估被证伪,新登记 H25<br>`2f34e61` 2026-09-24 15:12 docs(ARCHITECTURE §3 决策13): 候选层每源预配额——为什么限量再截断是免费的、宽池为何不许带 content_html、本地端为何不接（安全阀与策展策略不共用一个面）<br>`1128d72` 2026-09-23 20:30 docs(交付链重订+B136+BL13 核销): ①AGENTS §3 用户重订——eval 各命令降级按需工具(自评卷+未稳立锁=门禁分不出世界变了与作弊),新链=npm test→push→ci+Actions→云端实测→冒烟→对抗审查→文档同步,锁纪律三条(diff 带 why/红了报用户三选一/不许自消);EVAL_GUIDE+FEATURE_MATRIX+CLOUD_PIPELINE 同步暂缓头注 ②ISSUES:B136 cron-job 8430047 死 PAT 连跪 09-20 起停用(全天 2 轮/370 源到期),BL13 核销出账,B121 复验改探针 ③FEATURE_MATRIX §1.5 测试 604→609+如实更正 0 红误记,push-CI 缺口核销 ④ARCHITECTURE §3.6 双保险暂按一档读 ⑤INDEX 登记 RSS 设计文档(1)与交接文档,交接文档补三个裁定与交付进展 ⑥STAMPS 刷新;lint:docs 0 错 2 警 EXIT=0 |
...
| docs/FEATURE_MATRIX.md | 2026-09-24 | 一致 ⚠ | `b1e3f71` 2026-09-24 15:59 docs(级3 第一期生产读数到手·id=68): 两处原预估被证伪,新登记 H25<br>`f0affdb` 2026-09-24 15:00 docs(FEATURE_MATRIX 补级3能力行): §1.2 候选层每源预配额（四处部署面接线/本地端按裁定A不接/读数落 stats.prescreen）+ §1.5 头注与条数同步<br>`d2eefda` 2026-09-24 14:24 docs(步1 收口·四处读数更正): 交接文档快照 / ISSUES H19~H22 / 不变量20 / spec23 L4 作废标注 |
...
| docs/ISSUES.md | 2026-09-23 | 待判 ⚠ | `b1e3f71` 2026-09-24 15:59 docs(级3 第一期生产读数到手·id=68): 两处原预估被证伪,新登记 H25<br>`c913ef4` 2026-09-24 15:08 docs(ISSUES H24): ai_failed 冷却键单一全局位(120min)——初筛异常与通道全挂互相吞，09-23 两期同满足条件窗口内只落一条；且 detail 不存正文事后无法分辨子因<br>`f81bd6e` 2026-09-24 14:58 docs(ISSUES H23 登记): runDaily 裸报告 candidates 是门槛后口径且从不写 gateDropped（破不变量12，非本轮引入，故意不在步1 修以免污染归因） |
...
| docs/changes/2026-09-23-rss-pipeline-discussion-handoff.md | — | 不要求头注 | `66dade3` 2026-09-24 16:32 docs(收口链对账+实测边界写明): 读层两份生成器未活体打到,POST /api/daily-generate 待授权;H25 待拍板<br>`a228da1` 2026-09-24 16:25 docs(读层 HTTP 实测补做·收口链最后一步): /api/meta commit==origin/main,/api/daily 透 stats.prescreen 与直读库逐字段一致 |
...
| docs/eval/2026-09-24-prescreen-step1.md | 2026-09-24 | 不要求头注 | `66dade3` 2026-09-24 16:32 docs(收口链对账+实测边界写明): 读层两份生成器未活体打到,POST /api/daily-generate 待授权;H25 待拍板<br>`a228da1` 2026-09-24 16:25 docs(读层 HTTP 实测补做·收口链最后一步): /api/meta commit==origin/main,/api/daily 透 stats.prescreen 与直读库逐字段一致 |
...
| docs/specs/44-prescreen-tier/spec.md | 2026-09-24 | 不要求头注 ⚠ | `b1e3f71` 2026-09-24 15:59 docs(级3 第一期生产读数到手·id=68): 两处原预估被证伪,新登记 H25<br>`58d4b40` 2026-09-24 14:51 docs(spec44 遗留三条): perSourceCap 无后台写入口（PUT /api/settings 不含 prescreen 段）/ 历史行耗时不可比 / 步1 后仍会截断属预期 |
