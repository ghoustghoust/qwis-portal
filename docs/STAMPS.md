...
| docs/ISSUES.md | 2026-09-21 | 待判 | `40d98e0` 2026-09-22 11:45 feat(B8/P2-2)+test(MB1~MB4): 块级排版——mdBlocks 切分器(标题/无序/有序/空行分段)+MdRich 组件(块内仍走 MdText 内联解析,纯元素树无注入面),周刊/日报/热点详情长文位换用; DailyPage 外层 p→div(合法嵌套)<br>`718c7a1` 2026-09-22 11:35 docs(B73 核销入账): ISSUES 出队,线上实测读数与并行化+批量化的处置记进归档件<br>`b3d5552` 2026-09-22 11:15 fix(B10/P2-3,生产写): 线上字面键 daily.columns 从回声文案回写为 lib/daily-columns.js#DEFAULT_COLUMNS(备份 docs/eval/b10-daily-columns-backup.json + 写后回读逐条一致); 可见面等下一批日报复验,挪观察段 |
...
| docs/archive/debugging/2026-09-21-issues-round2-closed.md | — | 不要求头注 | `40d98e0` 2026-09-22 11:45 feat(B8/P2-2)+test(MB1~MB4): 块级排版——mdBlocks 切分器(标题/无序/有序/空行分段)+MdRich 组件(块内仍走 MdText 内联解析,纯元素树无注入面),周刊/日报/热点详情长文位换用; DailyPage 外层 p→div(合法嵌套)<br>`718c7a1` 2026-09-22 11:35 docs(B73 核销入账): ISSUES 出队,线上实测读数与并行化+批量化的处置记进归档件 |
