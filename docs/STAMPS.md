...
**统计**：底层文档 一致 6 · 待判 12 · 头部超前 0 · 缺头注 0　|　不要求头注 253 份　|　工作区未提交 2 份　|　总计 271 份
...
| docs/ISSUES.md | 2026-09-21 | 一致 ⚠ | `7b80501` 2026-09-21 23:52 fix(B131/P0-2)+feat(W24)+test(SC1~SC5): lib/db.js SCHEMA 与 ALTERS 补 translated_* 三列、server/db.js 补 translation_provider(新机建库写入即抛的缺口,生产靠 runner 自愈 ALTER 掩盖);新白盒 W24 两份建表源可建列集不许分叉+ALTERS 必进 SCHEMA 本体;F2P 按 B106 规则走「回退修复点」:基线红 2/2→改后绿(docs/eval/f2p/2026-09-21-b131-manual.json)<br>`1ab00b5` 2026-09-21 23:34 fix(B105/P0-1)+test(DR1~DR5): scratch 驱动族「退出段崩但载荷正确」不再翻红——共享 tests/driver-runner.js 把退出码与载荷分别判(spawnSync,无载荷才带 status+stderr 尾抛),11 处 execFileSync 调用点换用;清理误跑出的 10 个用例名垃圾库;全量 2 轮 549/549<br>`7210851` 2026-09-21 22:58 docs(ISSUES 待开发队列按阻塞级重排): P0 挡验收链/数据安全(B105→B131→⑥b→B132) → P1 口径(B29/B69/B23+B24/B133) → P2 体验(B73/B8/B10/B123/B17/B18/B21+H13) → P3 长尾(B87/B70/B86);审查列标注 ✅已放行/⏳待点头,用户逐条审;重排改写行原文已无损收进 round2 归档件 §九 |
...
| docs/archive/debugging/2026-09-21-issues-round2-closed.md | — | 不要求头注 ⚠ | `7b80501` 2026-09-21 23:52 fix(B131/P0-2)+feat(W24)+test(SC1~SC5): lib/db.js SCHEMA 与 ALTERS 补 translated_* 三列、server/db.js 补 translation_provider(新机建库写入即抛的缺口,生产靠 runner 自愈 ALTER 掩盖);新白盒 W24 两份建表源可建列集不许分叉+ALTERS 必进 SCHEMA 本体;F2P 按 B106 规则走「回退修复点」:基线红 2/2→改后绿(docs/eval/f2p/2026-09-21-b131-manual.json)<br>`1ab00b5` 2026-09-21 23:34 fix(B105/P0-1)+test(DR1~DR5): scratch 驱动族「退出段崩但载荷正确」不再翻红——共享 tests/driver-runner.js 把退出码与载荷分别判(spawnSync,无载荷才带 status+stderr 尾抛),11 处 execFileSync 调用点换用;清理误跑出的 10 个用例名垃圾库;全量 2 轮 549/549 |
