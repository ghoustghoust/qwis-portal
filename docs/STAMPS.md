...
| docs/ISSUES.md | 2026-09-21 | 待判 ⚠ | `2af6748` 2026-09-22 01:53 fix(B133/P1-4): #64-1 的「顶层读 .env」判据从行首缩进近似改走 src-spans 遮蔽视图(大括号深度定顶层),不再误伤锁里的反例样本字符串; 双向锁 #64-1b(样本不红/真违规红/函数体内不判)<br>`a22f4d1` 2026-09-22 01:42 fix(B69/P1-2)+test(AE1~AE3): 「报警有出口」判定收成 lib/alert-channels#usableChannels 一份——本地 getAlertSummary 与云端 /api/health/status 都改走它并带 usableChannelCount/noExit(云端另带投递态,不回显 URL)<br>`9cbb86d` 2026-09-22 01:35 fix(B29/P1-1 残余): 我的阅读视频侧两端改认「收藏或观看过」(READING_VIDEO_COND 进 lib/reading-filters,列表/计数/快路径三处同源;favorited 计数仍只认收藏); 连带把 watched_at 列收进两份建表源(此前由 videos 路由加载时顺手 ALTER); F2P=基线红 2/2→改后绿; N8/N9 期望表加观看未收藏样本 |
...
| docs/archive/debugging/2026-09-21-issues-round2-closed.md | — | 不要求头注 ⚠ | `2af6748` 2026-09-22 01:53 fix(B133/P1-4): #64-1 的「顶层读 .env」判据从行首缩进近似改走 src-spans 遮蔽视图(大括号深度定顶层),不再误伤锁里的反例样本字符串; 双向锁 #64-1b(样本不红/真违规红/函数体内不判)<br>`a22f4d1` 2026-09-22 01:42 fix(B69/P1-2)+test(AE1~AE3): 「报警有出口」判定收成 lib/alert-channels#usableChannels 一份——本地 getAlertSummary 与云端 /api/health/status 都改走它并带 usableChannelCount/noExit(云端另带投递态,不回显 URL) |
