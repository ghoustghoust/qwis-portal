...
| docs/archive/debugging/2026-09-21-issues-round2-closed.md | — | 不要求头注 | `51ec81f` 2026-09-22 01:14 docs(B132 核销入账): ISSUES 出队 + round2 归档件 §十一 补交付行; P0 队列全清<br>`306cc38` 2026-09-22 00:20 feat(⑥b/B101+B103 残余)+test(DC1~DC5): 转储凭证入库与两端接闸——lib/content-dump 新增 credentialFromManifest/credentialGate/deleteGateAny(磁盘优先、凭证兜底);dump:content --scope cloud 校验全过后把凭证写进 settings['retention.dumpCredential'](单独写连接);runner 读数改判凭证并带 via;云端 POST /api/data/cleanup 接闸(无凭证 409+审计行);CO2 与 W-5 两锁随新语义更新(措辞随腿变/隔离库先放凭证) |
