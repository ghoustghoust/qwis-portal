...
| docs/ISSUES.md | 2026-09-21 | 待判 ⚠ | `8e90a58` 2026-09-22 19:06 docs(B103②收口): /api/backup 只覆盖配置的语义写明——响应带 contentBackup 指路字段(内容表由内容级转储覆盖),RUNBOOK 备份段同步; 不建第二份备份面<br>`61a83b9` 2026-09-22 19:02 feat(B18/P2-6)+test(SC18-1~3): videos.score 落库链路——列进两份建表源+生产 ALTER; 'v'+id 深析条目回写收进 lib/score-persist.js 唯一实现(此前被 number-only 过滤整批跳过); 云端媒体栏带 score<br>`81eff31` 2026-09-22 18:53 fix(B70①②③⑤/P3-2): eval-content 三条判据质量(judge 定界+截断留痕/trend 原子写+损坏保护/golden 快照出 git)+三条形状锁改行为判据(B66-1 形态放宽/judge 纪律改 python 行为断言/覆盖项数补具名探针) |
...
| docs/archive/debugging/2026-09-21-issues-round2-closed.md | — | 不要求头注 ⚠ | `8e90a58` 2026-09-22 19:06 docs(B103②收口): /api/backup 只覆盖配置的语义写明——响应带 contentBackup 指路字段(内容表由内容级转储覆盖),RUNBOOK 备份段同步; 不建第二份备份面<br>`61a83b9` 2026-09-22 19:02 feat(B18/P2-6)+test(SC18-1~3): videos.score 落库链路——列进两份建表源+生产 ALTER; 'v'+id 深析条目回写收进 lib/score-persist.js 唯一实现(此前被 number-only 过滤整批跳过); 云端媒体栏带 score |
