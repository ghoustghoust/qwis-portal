...
| docs/ISSUES.md | 2026-09-21 | 待判 ⚠ | `c302c3f` 2026-09-22 11:05 feat(35B/B23+B24/P1-3)+test(SH1~SH5): 每源滚动尝试窗口+成功率公式落 lib/source-health.js 一份(三态 n/e/f 定长40+衰减+Beta 平滑+W<3 回 null),三端写入口同接,系统性故障不进分母; /api/health/source-stats 改真值并回显口径版本; 旧字段保留一个兼容周期<br>`2af6748` 2026-09-22 01:53 fix(B133/P1-4): #64-1 的「顶层读 .env」判据从行首缩进近似改走 src-spans 遮蔽视图(大括号深度定顶层),不再误伤锁里的反例样本字符串; 双向锁 #64-1b(样本不红/真违规红/函数体内不判)<br>`a22f4d1` 2026-09-22 01:42 fix(B69/P1-2)+test(AE1~AE3): 「报警有出口」判定收成 lib/alert-channels#usableChannels 一份——本地 getAlertSummary 与云端 /api/health/status 都改走它并带 usableChannelCount/noExit(云端另带投递态,不回显 URL) |
...
| docs/archive/debugging/2026-09-21-issues-round2-closed.md | — | 不要求头注 ⚠ | `c302c3f` 2026-09-22 11:05 feat(35B/B23+B24/P1-3)+test(SH1~SH5): 每源滚动尝试窗口+成功率公式落 lib/source-health.js 一份(三态 n/e/f 定长40+衰减+Beta 平滑+W<3 回 null),三端写入口同接,系统性故障不进分母; /api/health/source-stats 改真值并回显口径版本; 旧字段保留一个兼容周期<br>`2af6748` 2026-09-22 01:53 fix(B133/P1-4): #64-1 的「顶层读 .env」判据从行首缩进近似改走 src-spans 遮蔽视图(大括号深度定顶层),不再误伤锁里的反例样本字符串; 双向锁 #64-1b(样本不红/真违规红/函数体内不判) |
