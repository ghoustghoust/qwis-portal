...
**统计**：底层文档 一致 5 · 待判 13 · 头部超前 0 · 缺头注 0　|　不要求头注 253 份　|　工作区未提交 0 份　|　总计 271 份
...
| docs/ISSUES.md | 2026-09-21 | 待判 | `b3d5552` 2026-09-22 11:15 fix(B10/P2-3,生产写): 线上字面键 daily.columns 从回声文案回写为 lib/daily-columns.js#DEFAULT_COLUMNS(备份 docs/eval/b10-daily-columns-backup.json + 写后回读逐条一致); 可见面等下一批日报复验,挪观察段<br>`c814b9b` 2026-09-22 11:11 fix(B123/P2-4)+test(DH1/DH2): 早报页头排序文案读 stats.schemaVersion(AI 策展/关键词两档)+degraded 标记,不再无条件写「关键词规则排序」<br>`c302c3f` 2026-09-22 11:05 feat(35B/B23+B24/P1-3)+test(SH1~SH5): 每源滚动尝试窗口+成功率公式落 lib/source-health.js 一份(三态 n/e/f 定长40+衰减+Beta 平滑+W<3 回 null),三端写入口同接,系统性故障不进分母; /api/health/source-stats 改真值并回显口径版本; 旧字段保留一个兼容周期 |
...
| docs/archive/debugging/2026-09-21-issues-round2-closed.md | — | 不要求头注 | `b3d5552` 2026-09-22 11:15 fix(B10/P2-3,生产写): 线上字面键 daily.columns 从回声文案回写为 lib/daily-columns.js#DEFAULT_COLUMNS(备份 docs/eval/b10-daily-columns-backup.json + 写后回读逐条一致); 可见面等下一批日报复验,挪观察段<br>`c814b9b` 2026-09-22 11:11 fix(B123/P2-4)+test(DH1/DH2): 早报页头排序文案读 stats.schemaVersion(AI 策展/关键词两档)+degraded 标记,不再无条件写「关键词规则排序」 |
