...
**统计**：底层文档 一致 6 · 待判 12 · 头部超前 0 · 缺头注 0　|　不要求头注 253 份　|　工作区未提交 4 份　|　总计 271 份
...
| docs/ISSUES.md | 2026-09-21 | 一致 ⚠ | `1ab00b5` 2026-09-21 23:34 fix(B105/P0-1)+test(DR1~DR5): scratch 驱动族「退出段崩但载荷正确」不再翻红——共享 tests/driver-runner.js 把退出码与载荷分别判(spawnSync,无载荷才带 status+stderr 尾抛),11 处 execFileSync 调用点换用;清理误跑出的 10 个用例名垃圾库;全量 2 轮 549/549<br>`7210851` 2026-09-21 22:58 docs(ISSUES 待开发队列按阻塞级重排): P0 挡验收链/数据安全(B105→B131→⑥b→B132) → P1 口径(B29/B69/B23+B24/B133) → P2 体验(B73/B8/B10/B123/B17/B18/B21+H13) → P3 长尾(B87/B70/B86);审查列标注 ✅已放行/⏳待点头,用户逐条审;重排改写行原文已无损收进 round2 归档件 §九<br>`d452d52` 2026-09-21 22:47 docs(ISSUES 结构重排,用户口径): 已核销/已归档一律撤出正文——ISSUES 162→97 行,正文只剩『进度速览 + 未修/观察/等你操作/待裁决/挂案/阻塞』六段,每条只答现在坏着什么与下一步;撤出与改写行原文逐字进 round2 归档件 §九(无损校验过); test(G8 判据口径): B71~B83 登记面从单文件扩为 活文档+归档层调试件,查无某号仍红(追溯不断) |
...
| docs/archive/debugging/2026-09-21-issues-round2-closed.md | — | 不要求头注 ⚠ | `1ab00b5` 2026-09-21 23:34 fix(B105/P0-1)+test(DR1~DR5): scratch 驱动族「退出段崩但载荷正确」不再翻红——共享 tests/driver-runner.js 把退出码与载荷分别判(spawnSync,无载荷才带 status+stderr 尾抛),11 处 execFileSync 调用点换用;清理误跑出的 10 个用例名垃圾库;全量 2 轮 549/549<br>`9d6346b` 2026-09-21 22:58 docs(round2 归档件): 重复节号 §九 拆出 §十（阻塞级重排改写行） |
