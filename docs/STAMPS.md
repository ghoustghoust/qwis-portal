...
| docs/FEATURE_MATRIX.md | 2026-09-24 | 待判 | `fdf1fa0` 2026-09-25 02:38 docs(FEATURE_MATRIX): §1.5 条数 637→640 与 H31/H30 口径同步（单一写死处不许滞后）<br>`67a8b7a` 2026-09-25 01:18 docs(边界总表): Actions/Turso/AI 三侧的官方上限与本仓余量对账，并据此把 H27 的机制读法写完<br>`5329808` 2026-09-25 01:13 docs(H27-补·机制改判): schedule 事件会把 github.event.schedule 归属到别的串 ⇒ 门开在错误的 job 上 |
...
| docs/ISSUES.md | 2026-09-23 | 待判 | `68e7ec3` 2026-09-25 02:36 docs(H31 收口 + 交付链证据回填): /api/articles 的 limit 是"参数不存在"而非失效；本轮 CI/云端/冒烟/审查四步留证<br>`b104bed` 2026-09-25 02:28 fix(读层同形补到第 5 处 + 判据换成括号配平): 形态锁与执行锁各管一半，谁也别替谁背书（第三轮审查）<br>`fad1b5d` 2026-09-25 02:01 docs(第二轮复审核销): 41.6% 的分子其实是"7 天前存量"、ORDER BY RANDOM() 的"抽样"在读取侧等于全扫、H30 按读者口径是 5 天 0 次 |
...
| docs/changes/2026-09-23-rss-pipeline-discussion-handoff.md | — | 不要求头注 | `b104bed` 2026-09-25 02:28 fix(读层同形补到第 5 处 + 判据换成括号配平): 形态锁与执行锁各管一半，谁也别替谁背书（第三轮审查）<br>`f59ce97` 2026-09-25 01:01 feat(H30·早报导语归因入库): theme 为空的三条出口从"静默 null"改成可区分的 stats.themeSkip |
...
| docs/eval/2026-09-24-turso-read-amp.md | — | 不要求头注 | `68e7ec3` 2026-09-25 02:36 docs(H31 收口 + 交付链证据回填): /api/articles 的 limit 是"参数不存在"而非失效；本轮 CI/云端/冒烟/审查四步留证<br>`b104bed` 2026-09-25 02:28 fix(读层同形补到第 5 处 + 判据换成括号配平): 形态锁与执行锁各管一半，谁也别替谁背书（第三轮审查） |
