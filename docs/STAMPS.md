...
**统计**：底层文档 一致 6 · 待判 10 · 头部超前 2 · 缺头注 0　|　不要求头注 235 份　|　工作区未提交 17 份　|　总计 253 份
...
| docs/EVAL_GUIDE.md | 2026-09-19 | 待判 ⚠ | `99ba793` 2026-09-21 05:22 feat(B107)+test(N1~N9)+feat(W18): 噪声判定收成 lib/noise.js 一份（50 处命中→0），「我的阅读」两端默认排噪声<br>`ab4118d` 2026-09-21 04:15 feat(B127)+test(A1~A7): 过程层读数落进产物 + --report 先验输入 schema（喂错退 2，fail_env 也退 2）<br>`efb6ec0` 2026-09-21 03:56 feat(W9 收紧)+test(P1~P6): 坑号↔测试锁只认 test() 标题与断言消息 —— 注释与"#12 顶替 #1"都不再算覆盖 |
| docs/FEATURE_MATRIX.md | 2026-09-20 | 待判 ⚠ | `99ba793` 2026-09-21 05:22 feat(B107)+test(N1~N9)+feat(W18): 噪声判定收成 lib/noise.js 一份（50 处命中→0），「我的阅读」两端默认排噪声<br>`73147de` 2026-09-21 04:27 feat(B113/B110)+test(S1~S8)+坑#69: 凭据卫生收成一份共享实现，密钥扫描面从"只已跟踪"扩成两面<br>`ab4118d` 2026-09-21 04:15 feat(B127)+test(A1~A7): 过程层读数落进产物 + --report 先验输入 schema（喂错退 2，fail_env 也退 2） |
...
| docs/ISSUES.md | 2026-09-21 | 一致 ⚠ | `b42e6f1` 2026-09-21 05:46 docs(B107 验收轮入账): 两轮 e2e 读数 + 「含热榜」浏览器层证据（只提交 JSON，截图不入库）<br>`089b8c8` 2026-09-21 05:30 test(B107 端到端): 「含热榜」开关补浏览器层断言 + 本轮 F2P 证据与线上读数入账<br>`99ba793` 2026-09-21 05:22 feat(B107)+test(N1~N9)+feat(W18): 噪声判定收成 lib/noise.js 一份（50 处命中→0），「我的阅读」两端默认排噪声 |
...
| docs/specs/37-alerts-observability/37-2-alert-event-model-unify.md | 2026-09-21 | 不要求头注 ⚠ | `530a5f4` 2026-09-20 07:14 docs(specs+落账): 36/37/39/40 四域小 spec 补齐（26 份）+ 叫停轮的只读取证全部落档，未 push |
