...
| docs/archive/debugging/2026-09-21-issues-round2-closed.md | — | 不要求头注 ⚠ | `c814b9b` 2026-09-22 11:11 fix(B123/P2-4)+test(DH1/DH2): 早报页头排序文案读 stats.schemaVersion(AI 策展/关键词两档)+degraded 标记,不再无条件写「关键词规则排序」<br>`c302c3f` 2026-09-22 11:05 feat(35B/B23+B24/P1-3)+test(SH1~SH5): 每源滚动尝试窗口+成功率公式落 lib/source-health.js 一份(三态 n/e/f 定长40+衰减+Beta 平滑+W<3 回 null),三端写入口同接,系统性故障不进分母; /api/health/source-stats 改真值并回显口径版本; 旧字段保留一个兼容周期 |
