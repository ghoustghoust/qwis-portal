...
| docs/archive/debugging/2026-09-21-issues-round2-closed.md | — | 不要求头注 | `917d8f0` 2026-09-22 19:17 feat(B21+H13/P2-7)+test(HA1~HA5): 我的早报期号归档(同日原地替换/新日 max+1/封顶30/空态不占号) + /api/mybrief/archive 公开端点 + 页头第N期与往期列表; 日报期号按北京日去重读时算,页头带期号; 本期索引查实已有(吸顶 pill 导航)<br>`8e90a58` 2026-09-22 19:06 docs(B103②收口): /api/backup 只覆盖配置的语义写明——响应带 contentBackup 指路字段(内容表由内容级转储覆盖),RUNBOOK 备份段同步; 不建第二份备份面 |
