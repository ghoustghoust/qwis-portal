...
**统计**：底层文档 一致 5 · 待判 13 · 头部超前 0 · 缺头注 0　|　不要求头注 253 份　|　工作区未提交 3 份　|　总计 271 份
...
| docs/ISSUES.md | 2026-09-21 | 待判 ⚠ | `61a83b9` 2026-09-22 19:02 feat(B18/P2-6)+test(SC18-1~3): videos.score 落库链路——列进两份建表源+生产 ALTER; 'v'+id 深析条目回写收进 lib/score-persist.js 唯一实现(此前被 number-only 过滤整批跳过); 云端媒体栏带 score<br>`81eff31` 2026-09-22 18:53 fix(B70①②③⑤/P3-2): eval-content 三条判据质量(judge 定界+截断留痕/trend 原子写+损坏保护/golden 快照出 git)+三条形状锁改行为判据(B66-1 形态放宽/judge 纪律改 python 行为断言/覆盖项数补具名探针)<br>`3111efb` 2026-09-22 18:32 feat(B87/P3-1)+test(BP1/BP2): audit-cloud 加 B 站上游 nav 形状哨兵探针(code/data 契约,上游字段漂移先红),probe() 支持绝对 URL; 本轮实测探针 200 通过、巡检 21/22 |
...
| docs/archive/debugging/2026-09-21-issues-round2-closed.md | — | 不要求头注 ⚠ | `61a83b9` 2026-09-22 19:02 feat(B18/P2-6)+test(SC18-1~3): videos.score 落库链路——列进两份建表源+生产 ALTER; 'v'+id 深析条目回写收进 lib/score-persist.js 唯一实现(此前被 number-only 过滤整批跳过); 云端媒体栏带 score<br>`81eff31` 2026-09-22 18:53 fix(B70①②③⑤/P3-2): eval-content 三条判据质量(judge 定界+截断留痕/trend 原子写+损坏保护/golden 快照出 git)+三条形状锁改行为判据(B66-1 形态放宽/judge 纪律改 python 行为断言/覆盖项数补具名探针) |
