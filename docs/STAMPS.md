...
**统计**：底层文档 一致 7 · 待判 11 · 头部超前 0 · 缺头注 0　|　不要求头注 198 份　|　工作区未提交 0 份　|　总计 216 份
...
| docs/CLOUD_PIPELINE_GUIDE.md | 2026-09-23 | 待判 | `05fd6b8` 2026-09-25 00:06 feat(三臂行为回测给出"是否采用"的数) + 独立审查推翻本轮 3 条说法后逐条改写<br>`00fa95a` 2026-09-24 23:23 docs(H29·什么在大量读 Turso)：删除闸从 09-22 起恒挡→库只涨不删；体积不是腹泻源撑的<br>`48af997` 2026-09-24 21:00 feat(夜间预算按峰值重排 + 两段耗时/尝试数落库 + 配额键可写): 用户裁定"时间长一点都可以,各边界留缓冲" |
...
| docs/FEATURE_MATRIX.md | 2026-09-24 | 待判 | `e834df7` 2026-09-25 03:40 docs(FEATURE_MATRIX): 待办表补 H31/H32 两行（AGENTS §4 的入口表要与 ISSUES 活跃表同步）<br>`3b6f15e` 2026-09-25 03:11 test(第四轮复审核销): 提取器不再把注释/字符串里的 report 当返回点，I4 扩到五臂并诚实写下 consistent 的效力边界<br>`fdf1fa0` 2026-09-25 02:38 docs(FEATURE_MATRIX): §1.5 条数 637→640 与 H31/H30 口径同步（单一写死处不许滞后） |
...
| docs/ISSUES.md | 2026-09-23 | 待判 | `9da373c` 2026-09-25 03:33 docs(H32 + H27/H30 补实测): AI 批在 50 趟里只成 4 趟、实际墙钟 86~95min；主题全景 0 簇的病根是起名出口不是阈值<br>`3b6f15e` 2026-09-25 03:11 test(第四轮复审核销): 提取器不再把注释/字符串里的 report 当返回点，I4 扩到五臂并诚实写下 consistent 的效力边界<br>`68e7ec3` 2026-09-25 02:36 docs(H31 收口 + 交付链证据回填): /api/articles 的 limit 是"参数不存在"而非失效；本轮 CI/云端/冒烟/审查四步留证 |
...
| docs/changes/2026-09-23-rss-pipeline-discussion-handoff.md | — | 不要求头注 | `3b6f15e` 2026-09-25 03:11 test(第四轮复审核销): 提取器不再把注释/字符串里的 report 当返回点，I4 扩到五臂并诚实写下 consistent 的效力边界<br>`b104bed` 2026-09-25 02:28 fix(读层同形补到第 5 处 + 判据换成括号配平): 形态锁与执行锁各管一半，谁也别替谁背书（第三轮审查） |
...
| docs/eval/2026-09-24-boundaries.md | — | 不要求头注 | `9da373c` 2026-09-25 03:33 docs(H32 + H27/H30 补实测): AI 批在 50 趟里只成 4 趟、实际墙钟 86~95min；主题全景 0 簇的病根是起名出口不是阈值<br>`67a8b7a` 2026-09-25 01:18 docs(边界总表): Actions/Turso/AI 三侧的官方上限与本仓余量对账，并据此把 H27 的机制读法写完 |
...
| docs/eval/2026-09-24-turso-read-amp.md | — | 不要求头注 | `3b6f15e` 2026-09-25 03:11 test(第四轮复审核销): 提取器不再把注释/字符串里的 report 当返回点，I4 扩到五臂并诚实写下 consistent 的效力边界<br>`68e7ec3` 2026-09-25 02:36 docs(H31 收口 + 交付链证据回填): /api/articles 的 limit 是"参数不存在"而非失效；本轮 CI/云端/冒烟/审查四步留证 |
