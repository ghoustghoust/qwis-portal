...
| docs/ISSUES.md | 2026-09-21 | 待判 | `306cc38` 2026-09-22 00:20 feat(⑥b/B101+B103 残余)+test(DC1~DC5): 转储凭证入库与两端接闸——lib/content-dump 新增 credentialFromManifest/credentialGate/deleteGateAny(磁盘优先、凭证兜底);dump:content --scope cloud 校验全过后把凭证写进 settings['retention.dumpCredential'](单独写连接);runner 读数改判凭证并带 via;云端 POST /api/data/cleanup 接闸(无凭证 409+审计行);CO2 与 W-5 两锁随新语义更新(措辞随腿变/隔离库先放凭证)<br>`7b80501` 2026-09-21 23:52 fix(B131/P0-2)+feat(W24)+test(SC1~SC5): lib/db.js SCHEMA 与 ALTERS 补 translated_* 三列、server/db.js 补 translation_provider(新机建库写入即抛的缺口,生产靠 runner 自愈 ALTER 掩盖);新白盒 W24 两份建表源可建列集不许分叉+ALTERS 必进 SCHEMA 本体;F2P 按 B106 规则走「回退修复点」:基线红 2/2→改后绿(docs/eval/f2p/2026-09-21-b131-manual.json)<br>`1ab00b5` 2026-09-21 23:34 fix(B105/P0-1)+test(DR1~DR5): scratch 驱动族「退出段崩但载荷正确」不再翻红——共享 tests/driver-runner.js 把退出码与载荷分别判(spawnSync,无载荷才带 status+stderr 尾抛),11 处 execFileSync 调用点换用;清理误跑出的 10 个用例名垃圾库;全量 2 轮 549/549 |
...
| docs/archive/debugging/2026-09-21-issues-round2-closed.md | — | 不要求头注 | `306cc38` 2026-09-22 00:20 feat(⑥b/B101+B103 残余)+test(DC1~DC5): 转储凭证入库与两端接闸——lib/content-dump 新增 credentialFromManifest/credentialGate/deleteGateAny(磁盘优先、凭证兜底);dump:content --scope cloud 校验全过后把凭证写进 settings['retention.dumpCredential'](单独写连接);runner 读数改判凭证并带 via;云端 POST /api/data/cleanup 接闸(无凭证 409+审计行);CO2 与 W-5 两锁随新语义更新(措辞随腿变/隔离库先放凭证)<br>`7b80501` 2026-09-21 23:52 fix(B131/P0-2)+feat(W24)+test(SC1~SC5): lib/db.js SCHEMA 与 ALTERS 补 translated_* 三列、server/db.js 补 translation_provider(新机建库写入即抛的缺口,生产靠 runner 自愈 ALTER 掩盖);新白盒 W24 两份建表源可建列集不许分叉+ALTERS 必进 SCHEMA 本体;F2P 按 B106 规则走「回退修复点」:基线红 2/2→改后绿(docs/eval/f2p/2026-09-21-b131-manual.json) |
