...
→ **14:12Z 更正：等不来**，那一批没自动跑（成因见 `docs/eval/2026-09-24-source-diagnosis.md` §四 / ISSUES H27）；
  生产最新 AI 行仍是 `id=68`（`timeSplit` 之前写入）。所以 `timeSplit` 目前**只有沙盒执行证据（E3），没有生产读数**。

## 十七、配额键的读侧生产实测（09-24 14:13Z）

`GET /api/settings` → `prescreen = {"perSourceCap":2}`，且 `deployed commit == origin/main`（`7c47d93`）。
顺带自查（这个端点是**公开 GET**，我刚刚往响应里加了一段，所以必须回头验自己）：
响应顶层无 key/token/cookie/secret 命中的键名，整段响应里不出现 `sk-…`/`ghp_…` 样式 → 本次透出没有把凭据带出去。
未做：浏览器里登录后台看那一格并点保存（写生产 settings 未授权）；因此 UI 侧只有"构建通过 + P10 四头对齐锁 + 读侧真回值"，**不等于你已经在页面上看见它**。
