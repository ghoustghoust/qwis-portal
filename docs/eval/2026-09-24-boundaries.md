...
⇒ 主题全景（`themes` 0 簇）另有一笔账：用生产同一份贪心实现回放，**@0.45 至少 8 簇分布在 6 期**（⚠️ **下界** —— 回放只喂成栏的 46 条，线上 `:1255` 喂的是 `analyzed` 的 137~215 条），而库里 16 期只存下 3 簇 ⇒ 漏点不在阈值，而在"簇进库前要连过四条不留痕的出口"（`!r.ok`／无 JSON／`!name||!summary`／`JSON.parse` 抛）。**具体哪条在丢本轮没测出来**，因此不动阈值也不动出口，只登记 H32。
