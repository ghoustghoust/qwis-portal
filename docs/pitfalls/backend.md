...
### #72 删除闸指向的目录与转储产出的目录不是同一个 → 闸恒挡，看起来像"安全"，其实是把安全机制变成不可用（2026-09-21 接 B101 删除侧时实测抓到）
- 症状/诱因：把 B103 的 `deleteGate` 接进 runner `runCleanup` 时，我把转储目录写成 `data/content-dump`（父目录），
  而 `tools/dump-content.cjs` 的默认产出**按 scope 分子目录**（`data/content-dump/cloud`、`.../local`）。
  `manifest.json` 在子目录里 → 父目录永远判"没有内容级转储" → **闸恒挡**。
- 为什么这一类比假绿更难发现：它不报错、不打警告，行为完全"符合预期"（一条都不删）。
  只有在**真要删的那一天**才暴露，而那时人的第一反应是"这闸碍事"→ 直接绕过去 —— 安全机制反过来教会人绕过门禁（坑 #45/#64 同族）。
- 规则：①"不许做某事"型的闸必须配**双向锁**：坏输入必挡 **且** 合法输入必放行（只钉前一半 = 可以靠恒假通过）；
  ②闸读的目录/键名，必须与写它的那份实现**共用同一处定义或同一处约定**（这里靠测试里现造一份能通过校验的转储来钉：
  `makeDump` 用 `lib/content-dump` 自己的 `writeChunk`/`writeManifest` 生成，不手写清单形状 —— 手写形态漂移就是 #63 那族）；
  ③跨环境还要写明"这一端此刻必然被挡"是真的没有转储，而不是路径写错（runner 上没有本地转储目录，
  要放行得把 manifest 摘要搬进库里，登记为 B103 剩余项，不许用"改宽闸"来蒙过去）。
