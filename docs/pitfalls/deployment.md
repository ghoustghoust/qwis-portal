...
- 症状：线上 `/api/*` 全 500，用户先于我发现。而我最后一条线上读数是前一晚 22:2x，之后连续六轮全在本地做只读复核。
- 根因（**判法错误，不是遗漏**）：AGENTS §3 第 8 项被理解成"云端实测=测我这次改的东西"，于是零部署→不测→把交付链第 8 项标成"本轮没跑（合理）"。但存储配额、外部封锁、cron 停摆这类事**不看代码有没有动**，只按时钟发生。零部署恰恰是**唯一必须主动测线上**的情形，因为"没有新东西可测"= 没有任何东西在保护你。
- 规则：①**每轮交付至少一次线上活体探测**（`/api/meta` 一条即可），与有没有部署无关；②`BLOCKED`/配额类文案要**单独立一类**，不许混进"接口 500"；③存储侧封锁后**旧库不许删**——读封锁随配额重置解除，它是唯一回读来源（本轮 `settings.weekly.archive` 5 期归档就因"只搬本地副本"而暂时读不到）；④换存储的验收必须**两侧各验一次**：读路径（`/api/meta`）+ 写路径（一次 `workflow_dispatch`，看 `settings['cloud.collect']` 心跳与新库 articles 真增长），只看前者会漏掉整条采集链（实测：换 Secret 前 run `1072` 采集步 failure，换后 `1073` success）。
- 案例：`docs/ISSUES.md` B118/B119、`docs/RUNBOOK.md` §10.9、不变量 19。
