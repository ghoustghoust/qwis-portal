...
> 最后更新：2026-09-23（**specs 35~43 整批作废删除并留逐字反查锚点**；挂案按 09-23 只读探针复核：H16 云端线已交付但只 1/3 端、H12 的 `settings['weekly']` 键实为不存在、新增 H17 三项 P1 无视觉证据 / H18 生产 settings 残留 5 个测试键。上一轮的 结构重排 说明见 `docs/archive/debugging/2026-09-21-issues-round2-closed.md` §九）
...
| BL13 | 删除 65 份 spec 后，`npm test` 有 **2 条红**：`L3 锚点分母 >=100`（实扫 **47**）、`L5 裸文件名 >=20`（实扫 **12**）——删除前同一把锁读数是 **172 / 43**（`tests/regression-doc-lint-rules.test.js:46,55`） | **不是改坏了代码，是这两把锁在履行"分母塌了 = 判据可能被掏空"的职责**。不许为绿灯放松判据；三条候选待用户裁定：① 改扫固定夹具（语料与判据解耦）② 按 09-23 语料重设下限并把依据落档 ③ 判据本身不对（语料规模不是它能固定的量），摘掉 L3/L5 只留 `doc-lint --self-test` 的坏样本自证。**裁定前本轮不算交付完成** |
