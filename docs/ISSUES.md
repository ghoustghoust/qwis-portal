...
| H32 | **「主题全景」0 簇：阈值不是病根，而簇要连过四条不留痕的出口才进库**（H30 的姊妹项，本轮新量）：`tools/collect-turso.js#buildThemePanorama` 先按标题 token 的 Jaccard≥0.45 贪心聚簇、只留"≥2 条的簇"，再逐簇调模型起名，而簇要进库得连过**四条不留痕的出口**：`:1064 !r.ok → continue`、`:1065 匹配不到 {…} → continue`、`:1069 !name 或 !summary 缺一项 → continue`、`:1070 JSON.parse 抛 → catch{}`。⇒ 起名/解析任一环节一丢，**整簇直接消失**，`stats.themes` 于是记 0 簇，读起来却像"标题聚不到一起"（**到底是哪条在丢，本轮没测出来，不许指派主因**）。用生产同一份实现（逐字复制 + 函数文本漂移守卫）离线回放 16 期：**`@0.45` 至少 8 簇、分布在 6 期**，而库里这 16 期 `themes` 加起来只有 3 簇。⚠️ 这条读数是**下界**，两处偏差各自写明：① 回放喂的是**成栏后的 46 条**，线上真喂的是 `analyzed` 那 **137~215 条**（`:1255` 实测传参）⇒ 真实簇数只会更多；② 标题里存的是当前译文，翻译在批次之后还会补 ⇒ token 会变。阈值往下也不支持"放宽就能救"：0.45→0.25 只把 8 簇变 15 簇，而**最大簇始终只有 2~3 条**（放宽换来的是一堆两条一组的近似重复，不是真主题）；另有 3 期小批次（16~19 条）在 0.15 下仍 0 簇 —— 那几期的病根是**条目太少** | 本轮只登记 + 留可复现读数，**不动阈值、也不动 `if (!r.ok) continue`**。三条待拍：① 四条出口各接一个计数（与 `themeSkip` 同形：`stats.themePanoramaSkip={clustersFound,named,drops:{ai_failed,no_json,incomplete,bad_json}}`，零行为改动，先让"聚到几个、丢在哪一条"可查）；② 起名失败**不许吃掉整簇** —— 退化成"用簇内首条标题当主题名"要不要接受（读者会看到粗糙主题名，但比 0 簇强）；③ 条目少的期直接不出主题全景是否可接受（现在是空数组）。取证：`tools/_probe-theme-clusters.cjs`（只读、零模型） |
