...
| **B84** | **早报媒体栏的播客卡没有封面（用户 09-19 批注「没有图标」）**：`lib/media.js` 的口径是"cover 列命中音频特征 → `audio_url=cover`、`cover=null`，**由源头像/正文图兜底展示**"，`tools/collect-turso.js:1234/1442` 也按同一形状写。但 `MyBriefPage.jsx` 的 `MediaRow` 在 `cover` 为空时**只画了一个 56×103 的灰色块 + 🎧**（实测批注元素 bounds 正是 w-14 的兜底分支 219-221 行），注释里承诺的"源头像兜底"根本没实现。修法：无 cover 时用来源头像（页面已加载 sources，`SourceAvatar` 组件现成）铺满该槽位，播放徽章叠在上面；🎧/▶ 这类 emoji 字形在 Windows 上是单色描边，也要换成内联 SVG | 40 域（三报产物）+ 38-A 全局壳（图标体系）；与 B85 同批做 |
| **B85** | **日报页出现一个 996×594 的空心大框，正文只压在框底（用户 09-19 批注「奇怪的大框」）**：批注路径 `main > div > div:nth-of-type(5) > section:nth-of-type(3) > div > div > div`，元素内只有一行（rank 3 + 中英标题 + 标签 + 来源），但容器高度 594px、内容贴在最下方 → 高度来自某个"给封面/图占位"的固定尺寸或 `items-stretch` 的兄弟撑开，而图没渲染出来。**未定位到具体样式来源**（`DailyPage.jsx` 里没有 `<img>`/`aspect` 类名，说明这个框来自被复用的卡片组件），需要先抓一次真实 DOM 的 computed style 再改，不许凭猜删高度 | 36/40 交界（日报卡片渲染）；抓证据的命令见本条 |
