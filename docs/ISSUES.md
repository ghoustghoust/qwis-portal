...
| **BL12** | **Vercel 自动部署停摆**（2026-09-19 实测）：`git push` 连推 5 次（b766bf6 → 09482e9），production 仍是 04:25 那个 deployment，36 分钟没动；线上 `/api/hot/categories` 至今不含 `map` 即为铁证 | 本项目历史上「云端停摆 2 天无人发现」的同一形态**复发**。直接后果：本轮 B56 / B58 / B62 的**云端实测验收被卡**（AGENTS §2.2 要求线上实测才算交付），只有 04:25 之前的 B39/B53 真上线了 | 需人登录 Vercel 查 Git Integration（Push to Deploy 被关 / 构建队列配额 / 构建失败挂起）。**不在本机绕**：手动 `vercel --prod` 能推上去，但会把「触发器坏了」永久盖住——那是本项目最贵的自欺。已把这类漂移变成机器可查：`/api/meta` 回传 `VERCEL_GIT_COMMIT_SHA`，`eval:preflight` 新增「线上 commit == origin/main」硬判据（此刻它就是红的） |
