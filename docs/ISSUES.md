...
| **B93**（新，已修） | ✅ | 后台「RSS 最后同步」自上线起恒显示"从未同步"。根因：库里有 1 行 `sources.last_fetched_at` 是**字面字符串 `'null'`**（B15 同族的迁移期污染），文本序 `'null' > '2026-…'` → `MAX()` 取到它，读层归一化后又变回 `null`。修法：`MAX(NULLIF(last_fetched_at,'null'))` 在 SQL 层排掉。**范围说清（独立审查查出上一版"两端同口径"说过头）**：① 云端 `rss`/`bilibili` 两条与本地 `rss` 一条本来就有，本轮补的是**本地 `bilibili` 那条漏掉的 NULLIF**（同一手法，之前只修了一半）；② 本地 `lastSync` 的类型集合补齐成与云端一致的 `wechat/rss/wemp/x/youtube`（此前本地只查 `wechat/rss/x`，同一句话两端不同集合）；③ **`rssNextFetch` 仍是 `wechat/rss/x` 三型**，那是"下一次抓取时间"的另一条语义，不在本条范围内，没顺手改。**注意这是绕过污染、不是订正污染**：B15 的 24855 条 `'null'` 仍在库里，订正等授权。**锁**：I14 是行为锁（本地文件库里种"一行真时间戳 + 一行字面串 `'null'`"，端点必须回那串真时间戳，并带两条前提探针防"空库假绿"）；线上侧 `GET /api/status` 的 `lastSync.rss` 本轮实测已回 `2026-09-19T16:16:47.000Z`（此前恒 null） |
