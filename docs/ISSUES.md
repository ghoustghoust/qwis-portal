...
| **BL7** | **P0 云端报警链路无出口**（B44：渠道被回归测试写坏成 `url:undefined` 的 TEST，09-17 起 `sent:0`） | 系统正在"哑火"：熔断/停滞/AI 失败全都不再通知，与自愈改造互为前提（没有可观测就没有可信自愈） | ①`tools/sync-alerts-config.js --force` 从本地恢复真渠道（**写生产配置，需授权**）；②该测试改隔离库 + `after()` 加断言；③给 `settings.alerts` 写路径加"禁止测试库指纹"守卫 |
| **BL8** | **P0 `ai.minIntervalMs=0`**（B49） | 无间隔硬打 Agnes 免费池 → 45-60 分钟耗尽配额（坑 #A1），早报/周刊/翻译互相抢额度 | 复位 4000 并加"低于 1000 视为误配"下限保护（三端同一实现） |
| **BL9** | **P0 `settings.ai` 仍可被后台写回**（B50），与 env-only 不变量冲突 | 09-11 全链路 401 停摆 2 天的合法复发通道 | 需裁决：保留可写（改文档 + 加审计与变更告警）或改只读（引导去 Vercel env）；见 `docs/specs/39-ai-console/spec.md` F1 |
| BL10 | `'null'` 字符串污染 2.5 万行（B15 订正） | 毒害保留清理豁免、未读角标、阅读足迹三处口径；不先清则任何"未读/已读"统计与自动回收都不可信 | 一行修 `migrate-to-turso.js:250` + 一次性 `UPDATE ... WHERE read_at='null'`（**写生产库，需授权**） |
