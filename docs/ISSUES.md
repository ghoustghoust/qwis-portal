...
> 最后更新：2026-09-20（B8~B26 批次过**第二轮独立对抗审查**：三条"核销"被证伪并回退——B14 的 `digest.readCount=24869` 里 98.8% 是 `read_at='null'` 脏值、B20 门槛只接在 5 个写入点里的 1 个（线上 `id=103` 实测仍有 29/22 分条目）、B8 的措辞把字段认错了且块级 markdown 仍不支持（拆出 B92）。顺藤补出门槛全接 + B93（`'null'` 毒 `MAX()` 致「RSS 最后同步」恒空）+ 坑 **#57**（CRLF 让文本锁与注入探针静默失效）/ **#58**（手工清单判据必漏，改从写入点反查）。活跃 B8~B93、观察 W1~W10、挂案 H1~H17）
...
| **B94**（新） | 待办 | 来源名里带着**未解码的 XML 实体**上屏：实测 `sources` 表 1437 行里 **11 行**名字含 `&amp;` / `&quot;` / `&apos;`（如 `Hacker News - Newest: &quot;AI&quot;`、`Reddit · Linux, Self-Hosted, DevOps &amp; AI`、`Oragekk&amp;apos;s Blog` ← 这条是**双重转义**），前端按文本渲染，用户看到的就是 `&quot;`。不是死角：本轮线上 `GET /api/status/daily-sources` 的 Top3 里 **2 条被污染**（来源榜就在阅读器右侧）。根因位点在 OPML/订阅导入未解码实体（`parseOpml` 一路），且三端各有一份导入实现（AGENTS §1 同族）。修法要一次覆盖：①导入侧解码 ②存量 11 行订正（生产写，等授权）③显示侧兜底（`cleanNull` 同族的一个 `decodeEntities`）|
...
| W16 | 本轮两次"跑一次红、复跑全绿"的观察项（**都不是产品红**，但两次都没把失败读数留住，处置不合格） | ①`npm test` 全量里 `tests/regression-cloud-sources.test.js`「11. DELETE 源 → 级联删除」红 **1/5 次**（17:49、17:52 两次全量中的一次红，17:55/18:01/18:03 三次全绿；单独跑该文件 **3/3 绿**）。红的那两次都发生在**端到端浏览器轮并发**期间 → 与 W10 同族（负载/DNS/并发时序），判 `fail_env`。②`node tools/audit-cloud.js` 在部署刚完成时出现 **19/1**（`/api/videos` 同轮读数 12.9s、`/api/status` 7.8s，冷启动特征），随后 **3/3 全绿 20/0/1** → 判冷启动窗口，不算产品红。<br>**我自己的不合格之处**：两次红都没抓到失败项的断言原文/两侧读数（第 ①次 grep 过滤把 AssertionError 段滤掉了），只留下"复跑就绿"的结论——这正是坑 #53 规则③禁止的行为（红必须带两侧读数）。**下次遇到必须先存整段输出再复跑**（`> docs/eval/…txt`），不许用"复跑绿了"当结论；本条按 §3.4 的三连跑口径已计数，若再次复现且能取到读数，升级为独立 bug。 |
