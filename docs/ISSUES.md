...
| H27-补 | **09-24 17:10Z 两条新样本把 H27 的机制钉得更死：不是"抢队列"，是 `github.event.schedule` 串等值这道门本身会开错 job**。逐 run 读 jobs（只读 API）：`run 35964359306`（event=schedule，created **06:24:47Z**）里 **只有 `snapshot` 跑了**，`collect / daily-report / daily-ai / daily-ai-evening / cleanup / weekly / mybrief` 全 skipped —— 而 `snapshot` 的门是 `github.event.schedule == '33 1 * * *'`（01:33Z）：**06:24Z 到达的事件被归属成了那条凌晨的串**（要么是一条 01:33 事件迟到 5h 才落地，要么归属错版，两种都致命）。对照组 `run 35963578415`（06:14:52Z）归属正确（跑 collect，:07 线迟到约 7 分钟）。⇒ 15 分钟线基本能到（迟到几分钟），**每日一根的线要么不来、要么来了挂到别的串上**；这也解释"同流 collect/snapshot 的门照样开"——它们开着，但**开在错误的时刻** | **修法结论收紧**：AI 批的门**不许依赖 `github.event.schedule` 串等值**。两条都可行、都可回退：(a) **一个 workflow 文件只留一条 cron**（等值判断天然唯一，串不可能挂错）；(b) **搭在已证明 100% 可靠的 `workflow_dispatch` 通道上**，在步骤内按**北京时间窗**判"该不该跑"+"库里今日有没有 AI 行"，跑过写 claim 键挡重（幂等自检）。(a) 是纯搬迁零新逻辑，(b) 才真正免疫 schedule 丢失。**仍等你选**，与 H29/H30 不冲突
  **09-24 夜补一批"门到底开了几次"的分子分母（逐 run 拉 jobs，只读：`GET /actions/runs?event=workflow_dispatch` + `event=schedule` 各 25 条，再对每个 run `GET /actions/runs/{id}/jobs`；**取证脚本不落库**，因为它要从 `docs/HANDOVER.md` 取 PAT —— 本仓是公开仓，指向凭据文件的脚本本身也不许进历史）**：扫 collect.yml 最近 **50 个 run**（dispatch+schedule 混样；`collect=39 success + 11 skipped = 50` 自洽）⇒ 真跑成功次数 **collect=39、daily-ai=2、daily-ai-evening=2、daily-report=2、cleanup=2、weekly=0、mybrief=0、snapshot=2**，其余全是 `skipped`。
  ...
  ⇒ ⇒ **上面这两条同夜即被第五轮审查判错，我 20:12Z 自己重跑（`tools/_probe-ai-runs-vs-rows.cjs`，只读）确认审查对**：
  ① 分母错了 —— 两个 AI job 的门是 `collect.yml:109/134 → schedule==… || (dispatch && mode==daily-ai)`，**dispatch 侧（默认 mode=collect）结构上不可能跑 AI 批**，把 25 趟 dispatch 混进来等于给分母灌注定不可能的样本。
  **按真实点位重算**：schedule 侧 09-12 起 `daily-ai`+`daily-ai-evening` 共 **23 次 success**，两根每日 cron 自 09-13 存在 ⇒ 应有 ~24 根、实得 23 根 = **96% 触发率**。⇒ **"AI 批长期没有触发口"这句作废，AI 批基本天天在跑。**
  ② "墙钟与 `elapsedMin` 对得上"是**拿两个不同口径硬凑**：`elapsedMin` 只算 `runDailyAi` 内 `t0→INSERT`（`:1139/:1441`），job 墙钟另含 checkout/`npm ci`/回写；配对实测两组：id=70 job 113.0min vs `elapsedMin=108.4`、id=68 job 94.1min vs `87.4` ⇒ 差 **3.7~6.7min 的固定 runner 开销**。而我引用的 86/88/95 三趟对应的 `elapsedMin` 是 8.2/8.3/9.0 —— 那正是 **B137 的 ÷10 单位坑**（真值 82/83/90），我自己记录过这个坑却没换算。
  ③ 余量口径：**最长实测 113min** ⇒ `BUDGET_MS=300min` 是 **2.8×**（job 口径 2.65×），不是 3.5×（那是拿三个数里最小的 86 除的）。
  ④ `weekly=0`/`mybrief=0` 也被判错：weekly 的门是 `:185 → 3 10 * * 5`（每周五），我那个 09-22~09-24 的窗口里**一次都不该跑**（全历史回读它在 schedule 侧 success 1 次 = `run 35355654475 / 09-18T14:20:52Z`，10:03 点位迟到 4h17m 仍开了 ⇒ 1/1）；`mybrief` **根本没有 cron**（`:160` 只认 `dispatch && mode=='mybrief'`）⇒ "应跑 0 次"，把它算进病灶是同义反复。
  **仍然成立的部分（别连带丢掉）**：迟到与丢点是真事 —— 15 分钟线实测投递 ≈**7%**（175 次 collect success ÷ 25.8 天 ≈ 6.8 次/日，应 96 次/日；比先前写的 12% 更差），每日单根线（daily-report/snapshot/cleanup）各 13 次 ≈ **50%**；`run 35964359306` 的"归属到别的串"证据也没被推翻。⇒ **修法优先级要重排**：H27 原来那三个候选对付的是已经 96% 的两根每日线，真瓶颈是**增量线丢 93%** 与偶发错版。
...
| H30 | **早报的「主题导语 / 主题全景」从来没真正产出过：全库 57 期早报里只有 1 期带导语，且三条失败出口都不留痕**（口径按 09-24 夜独立审查补齐，先前只报了最轻的那一层）：① **期口径** —— AI 档（`schemaVersion>=2`）16 期 → theme 缺失 **15 期（94%）**；② **北京日口径**（"期"不是读者看到的单位，同一个北京日最多有 5 期 AI 档）→ 5 天里 **4 天（80%）当天所有期都无导语**；③ **读者口径**（真判据是 `pickDailyReport`：`rows.find(是 AI 档 且 age≤30h) || rows[0]`，`lib/brief-guards.js:40`）→ ⚠️ 我第一版拿"当天 id 最大的那一期"当代理，得出"**5 天 0 天带导语**"，**这句被第三轮审查按真判据回放证伪**（它以 15 分钟步进把全 57 行重放过 `pickDailyReport`）：全库唯一带导语的 `id=49 / 2026-09-20T03:34Z` 在被下一期顶掉之前，**北京 09-20 约 11:45~12:00 那 15~30 分钟里读者确实看到过导语**。正确说法是"**5 天里只有 1 天、且只有几十分钟的窗口出现过导语**" —— 仍然糟，但不许写成"从没有过"。另 `themes` 主题全景 **14/16 期是 0 簇**；而 `api/_ai.js#generateTheme` 的三条 `return null`（`!r.ok` / 每行都被污染判据否决 / 挑出的那句被一票否决）**既不写日志也不入库**，runner 那侧的 `.catch()` 只在**抛错**时才出声 ⇒ 事后完全分不清"模型没答"与"我们自己把答案判死了"。这正是 FEATURE_MATRIX 里挂着 ✅（18-daily-ai-v2 / T3-1 R0c）而读者侧长期缺功能的形态 | **本轮只做了"看得见"这一半**（零产品行为改动）：`generateTheme` 拆出 `generateThemeDetailed` + 纯函数 `pickThemeReply`，日报写入器在 theme 为空时把 **`stats.themeSkip = { why, err?, detail?, lines? }`** 落库，契约收录，锁 **T1~T4**（`tests/regression-theme-observe.test.js`）钉"三态可区分 + 旧字符串契约不坏 + runner 必须落归因"。⚠️ **归因只覆盖早报 AI 档，周刊与我的早报仍在静默**（09-24 夜独立审查指出本条先前没写范围，容易被读成"导语问题已收口"）：`tools/collect-turso.js:964`（周刊）与 `:1631`（我的早报）调的还是 `generateTheme` 的字符串投影 ⇒ **同样三条 null 出口一条没接归因**；它们不写 `daily_reports.stats`，落点是 `settings['weekly.latest']` / `settings['mybrief.latest']`，要接得先定"归因放哪"（本期不动）。**同形这一半在部署面上补到底（第四轮审查后标明范围：本地灾备端 `server/` 按用户 09-24 裁定 A 不在部署面，同型缺陷在那里仍开着 —— `server/services/ai/daily.js:399/409` 两份返回都不带三字段，而 `web/src/pages/DailyPage.jsx:175` 读的是同一个 `report?.theme`）**：部署面共五处回早报对象（`api/[...slug].js` 的新鲜/自动生成/过期/手动重算 + `api/daily-generate.js` 的写入回执），前四处原先漏字段、第五处连字段都没有，现已全部带 `theme/schemaVersion/degraded`；判据从"锚点后 700 字符含字样"换成**括号配平取字面量 + 只认顶层键**（`objectLiteralKeysAt`），并加执行锁 **I4**（`file:` 临时库上把四条分支各跑一遍，断言值真从 `stats` 来）。三条坏样本实测：字段嵌进子对象 → 红；只留一行含字样的注释 → 红；键在但取错来源（`stats.themeX`）→ 形态判据**仍绿**、是 I4 红 ⇒ 形态锁与执行锁各管一半，谁也别替谁背书。⚠️ 顺带记一次判据自打：新加的派生锁 **P12 第一版看不见条件展开**（`...(cond ? {} : { themeSkip: … })`），是它自己的坏样本自证把这层漏网暴露出来的（已修：圆括号当透明、只取分支对象第一层）。**下一刀等你拍**：为什么 94% 无导语 —— 是 `ai_failed`（配额/超时/模型换名）还是 `all_lines_rejected`（判据过严，例如行长 >120 与"必须只谈内容"这对约束把正常答案打回）？**跑一期 AI 批就有 `themeSkip.why` 就能定**；在那之前不许先动阈值。另：`themes` 0 簇的阈值（Jaccard≥0.45 且簇≥2）同样未测"改前改后各能出几簇"，也不许先动。取证：`tools/_probe-ai-feature-presence.cjs`（只读；09-24 夜补了三层口径 —— 按北京日滚动、"当天 id 最大那一期"、以及全库带导语期逐条点名。只报"期口径 94%"会两头失真：比率被同期数放大（同一天最多 5 期），而"读者到底读到过没有"要按 `pickDailyReport` 真判据回放才说得清（本轮第一版在这里用错了代理，已被第三轮审查更正）
  ...
  ⇒ ⇒ **同夜被第五轮审查判错，我复跑（`tools/_probe-ai-runs-vs-rows.cjs`）确认它说得对，三处更正**：
  ① "15 期都有 117~215 条真深析"**是错的** —— 其中 4 期是 0/23/25/32 条（id=48/50/51/53），只有 11 期落在 117~215；
  ② 更要紧的是**"排除模型整体不可用"这句本身作废**：等到的那期真归因（**id=70，09-24T19:44Z，全库第一条带 `themeSkip.why` 的**）恰恰是 `why=ai_failed`，
     `err="agnes 仅含 reasoning 无 content (finish=length, head=用户希望我作为科技媒体主编，根据给出的 30 篇入选文章标题与推荐理由，用一句话概括今…)"`，而同期 `analyzed=198` ——
     **深析 198 条全通、唯独起名这一发被模型只回思维链 + `finish=length` 打死**（`maxTokens=300` 对推理模型不够），这正是 **B121② 的已知形态**（周刊杂志结构那条已经用"退避重试一次"治过它）。
     ⇒ 所以主嫌不是清洗判据，而是**导语这一发自己的调用形状**；H30 原本的"三条静默出口"现在有一条被真数据点着了。
  ③ "空壳批次"那句的**分母也要降级**：id=48/49/50/53 里有三期是 09-20 的人工 dispatch 补跑（`run 35486892144` 03:33→03:39 对上 id=49 的 03:34:18），
     另两期在 schedule+dispatch 两侧**找不到任何 run** ⇒ "16 期 / 94% / 14-of-16"这些比率都是**在生产批与人工批混合的总体上算的**，不许当纯生产触发率引用。
  **仍然成立**：0 是真 0（`filterStats` 键确实在库里，值是字面 `{"candidates":0,"passed":0,"analyzed":0}`，不是缺键被读成 0）；09-20~09-22 那 11 期缺 `failed` 键、09-23/24 有 5 期带（我先前写"8 期缺/4 期有"也是数错的）；"阈值不许先动"这条纪律不变。 |
| H32 | **「主题全景」0 簇：阈值不是病根，而簇要连过四条不留痕的出口才进库**（H30 的姊妹项，本轮新量）：`tools/collect-turso.js#buildThemePanorama` 先按标题 token 的 Jaccard≥0.45 贪心聚簇、只留"≥2 条的簇"，再逐簇调模型起名，而簇要进库得连过**四条不留痕的出口**：`:1064 !r.ok → continue`、`:1066 匹配不到 {…} → continue`、`:1069 !name 或 !summary 缺一项 → continue`、`:1068 JSON.parse 抛 → :1076 catch{}`（⚠️ 第五轮又数出**两条更早的静默点**：`:1040 items.length<2 直接 return []`、`:1047 标题切不出 token 的条目 continue` —— 后者意味着"标题里没有 ≥2 字词"的条目**根本不进簇**，与阈值无关）。⇒ 起名/解析任一环节一丢，**整簇直接消失**，`stats.themes` 于是记 0 簇，读起来却像"标题聚不到一起"（**到底是哪条在丢，本轮没测出来，不许指派主因**）。用生产同一份实现（逐字复制 + 函数文本漂移守卫）离线回放 16 期：**`@0.45` 至少 8 簇、分布在 6 期**，而库里这 16 期 `themes` 加起来只有 3 簇。⚠️ 这条读数是**下界**，两处偏差各自写明：① 回放喂的是**成栏后的 46 条**，线上真喂的是 `analyzed` 那 **137~215 条**（`:1255` 实测传参）⇒ 真实簇数只会更多；② 标题里存的是当前译文，翻译在批次之后还会补 ⇒ token 会变。③ **第五轮又补两条**：(a) "生产同一份贪心实现"这句里的**漂移守卫是半恒真** —— 它只比 `titleTokens`/`jaccard` 两段函数文本，实测把生产改坏（阈值 0.45→0.90、`items>=2`→`>=3`、`slice(0,4)`→`slice(0,1)`）守卫**全不红**，只有改坏函数本体才红 ⇒ 它守的不是"贪心等价"，措辞要降级；(b) **单调性有反例**：id=70 回放 0 簇、库里却存着 1 簇 ⇒ "成栏 46 条"与 `analyzed` 不是包含关系，**"8 簇减 3 簇 = 丢了 5 个"这类逐期加减不合法**。阈值往下也不支持"放宽就能救"：0.45→0.25 只把 8 簇变 15 簇，而**最大簇始终只有 2~3 条**（放宽换来的是一堆两条一组的近似重复，不是真主题）；另有 3 期小批次（16~19 条）在 0.15 下仍 0 簇 —— 那几期的病根是**条目太少** | 本轮只登记 + 留可复现读数，**不动阈值、也不动 `if (!r.ok) continue`**。三条待拍：① 四条出口各接一个计数（与 `themeSkip` 同形：`stats.themePanoramaSkip={clustersFound,named,drops:{ai_failed,no_json,incomplete,bad_json}}`，零行为改动，先让"聚到几个、丢在哪一条"可查）；② 起名失败**不许吃掉整簇** —— 退化成"用簇内首条标题当主题名"要不要接受（读者会看到粗糙主题名，但比 0 簇强）；③ 条目少的期直接不出主题全景是否可接受（现在是空数组）。取证：`tools/_probe-theme-clusters.cjs`（只读、零模型） |
| H33 | **导语的第一条真归因到了，而且指向一处可修的调用形状**（09-24 20:12Z 实测，H30 的下游）：`id=70`（09-24T19:44Z，AI 档，`analyzed=198`）落出全库第一条 `stats.themeSkip` ⇒ **`why=ai_failed`**、`err="agnes 仅含 reasoning 无 content (finish=length, head=用户希望我作为科技媒体主编，根据给出的 30 篇入选文章标题与推荐理由，用一句话概括今…)"`。读法：**深析 198 条全通，唯独导语这一发只回思维链就被 `maxTokens=300` 截断** ⇒ 与 **B121② 同一形态**（周刊杂志结构那条为同样的形状已经加了"退避 15s 重试一次"，见 `tools/collect-turso.js:970` 附近），而导语这条**没有重试、也没有单独的 token 预算** | 三条候选（都可回退、都需要你点头才动，因为它们改的是产品行为/配额）：① **给 `kind:'theme'` 加重试**（与周刊同款：一次退避重试，代价 60~75s，专治 reasoning-only + `finish=length`）；② **抬导语的 `maxTokens`**（300 → 600~800；⚠️ 这是配额动作，未测真实消耗，且推理模型可能把增量全花在链上照样不出正文 ⇒ ①比②更像对症）；③ **换非推理模型跑导语**（本项目 `api/_ai.js` 已有多模型分支，但换名会连带改判据的适用面，属独立议题）。**本轮只做了一件**：把这条真读数如实入库（此前所有"94%/为什么"的争论都缺这一颗钉子）。取证：`tools/_probe-ai-runs-vs-rows.cjs`（只读） |
| H34 | **09-12~09-19 有 14 次 AI job 在 CI 里 success，新库里却没有对应行；而老库现在读封锁，无法回查**（09-24 20:12Z 实测，本轮新查）：`GET /actions/runs?event=schedule` 逐 run 拉 jobs ⇒ schedule 侧 `daily-ai`+`daily-ai-evening` 自 09-12 共 **23 次 success**，其中 **09-20 起的 9 次都能在 `daily_reports` 对上 4~6 分钟内的 AI 行**，而 **09-12~09-19 的 14 次一次都对不上**（新库 AI 档最早 = `id=48 / 09-20T03:02Z`；那几天新库里只有 4 行关键词档、09-15~09-18 整四天 0 行）。**为什么现在判不了性质**：去老库回读同一批日期 ⇒ `BLOCKED: SQL read operations are forbidden`（`tools/_probe-old-db-ai-rows.cjs`，只读双库对照）（**B118 的读封锁至今还在**，这正是"老库必须留着、它是唯一回读来源"那条决定当前的代价）。第二种自证也试过了（20:18Z 实测，`tools/_probe-collect-heartbeat-window.cjs`）：runner 自己的写侧心跳 `settings['cloud.collect'].history` **只剩 168 条、最早一条落在 09-22 之后** ⇒ 09-12~09-19 那 14 批的心跳**已经被滚动窗口转掉了**，含 daily-ai 字样的整库只剩 3 条。⇒ **两条回读路都断了，只剩"老库解锁"一条**。两种解释都还活着：(a) 那 14 次写进了当时的老库、换库时没带过来 ⇒ 只是**迁移边界**，不是丢写；(b) 当时就写到新库却行不见了 ⇒ 那是**写侧真洞**，比 H27 更靠上游。**副产物一条可观测性缺口**：心跳只留 168 条（≈2.5 天）⇒ 任何"上周有没有跑过/失败过"的问题都无法回答 | **本轮不猜，只登记 + 给判定路径**：① 老库解封锁（升级/换月配额）后跑同一份对账脚本，能查到行 = (a) 收口；② 若要把这类对账做成长期能力，得先给心跳加**按日汇总的常驻键**（不是把 168 改大 —— 那是写放大），待拍板；③ 若两条都指 (b) ⇒ 升 P0。**别顺手做的事**：为"补回那 14 期"重跑生成器 —— 那会写生产库，且窗口已过、结果不是当时那份 |
 **本轮不猜，只登记 + 给判定路径**：① 老库解封锁（升级/换月配额）后跑同一份对账脚本，能查到行 = (a) 收口；② 或查 runner 的写日志/`settings['cloud.collect'].history` 里那 14 个批次的 `daily-ai` 条目有没有报过 write error（这一步我可以只读做，下一轮）；③ 若两条都指 (b) ⇒ 升 P0。**别顺手做的事**：为"补回那 14 期"去重跑生成器 —— 那会写生产库，且窗口已过、结果不是当时那份 |
