...
> **B8~B26 已于 2026-09-19 深夜按条处置完毕，逐条状态与证据见下方「B8~B26 本轮处置」**
> （上面这张表保留原始登记与定性，不再逐行改写，避免同一事实两处漂移）。
> 处置中新登记出 B88 / B89 / B90 三条，列在该表末尾。
...
| # | 结论 | 一手证据 / 剩余动作 |
|---|---|---|
| **B8** | ✅ | 真因不是"没接 markdown 组件"，而是**同一字段混两种来源**：采集正文是 HTML、AI 摘要是带 `**`/`==` 的纯文本，而三处回退无脑走 `safeHtml` → 纯文本被转义、标记变裸星号。新增 `ui/RichText.jsx` 按"像不像 HTML"分流（保留调用方 class 与 ref），接进文章详情 / 快速学习弹窗 / 热点详情（导读+推荐理由）。锁 I9（F2P base `41d6af2` 红 → 绿）。**未做线上复验**：需要一条 summary 真含 md 标记的样本，本轮没在云端找到，故只到"契约已锁 + 构建通过"这一层 |
...
| **B12** | ✅ 核销 | 4 项红在 BL1 轮已重锚到根树真文件；本轮实测 `npm test` **417/417、0 红、0 跳过** |
| **B13** | ✅ 核销 | 全程 `线上 commit == origin/main` 实测一致（最近一次 `2ecf3d4`） |
| **B14** | ✅ 核销 | `GET /api/mybrief` 现带 `digest`（`readCount:24869 / laterCount:1 / topSources[…]`）。**注意**：这个 24869 是被 B15 的 `read_at='null'` 脏值毒害出来的数，别拿它当"真读了 2.5 万篇" |
...
| **B20** | ✅ | 门槛**已生效**：线上 AI 版（`schemaVersion=2`）46 条里 min score = **30**，正落在 `passesDailyQualityGate` 的门槛上；当初那条 score=10 的「出售 AI 工作站」不再入报 |
...
| **B25** | ✅ | 两处未提交在途改动已提交（B8 的 DailyPage 主题 MdText + B11 的 TabLoader）；原先记的 `index.css` 注释破口（`*/.rail-btn`）实测早已修好 |
