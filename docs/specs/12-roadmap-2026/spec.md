...
### P1-2 AI 基础设施
- **评测驱动**（借鉴 Aliyun ACP RAG 教程 2026-09-12）：AI 功能必须带"黄金集评测"——固定 20-30 篇已知优劣的文章作为评测集，初筛器/评分器的每次 prompt 调整都跑一遍对比分数分布，防止"改了 prompt 以为变好了"
- **长文 Markdown 结构感知切片**（BestBlogs + RAG 教程同源结论）：>6000 字符的文章按标题/代码块/表格边界切段，每段 ≤3000 字符——供摘要/翻译复用
- **检索增强思想移植**（不做向量库）：搜索支持查询改写（同义词扩展）+ 标签加权召回；推荐算法用"初筛召回→重排序"两段式
