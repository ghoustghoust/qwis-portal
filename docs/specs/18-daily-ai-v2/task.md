...
> **事后更正（2026-09-20，B120）**：T6 的五条回归用例里**没有任何一条管条目带不带封面图**，而 T7 写了"前端截图验收"却**从未落成 `checklist.md` 的核对项**（实测该文件里没有"截图/验收"字样）——计划里有、清单里没有，于是它静默消失。结果：这一版从 09-13 落地起，`/daily/` 的 `<img>` 数量恒为 **0**（runner 的 `fmt()` 投影漏 `cover`），而"限量真实生成实测通过"与 T6"全绿"照样都记成了达成。
> 修复与取证见 `docs/ISSUES.md` B120；**"接口 200 与用例全绿都不算验收"**这条口径见 `docs/CLOUD_PIPELINE_GUIDE.md` 不变量 19 与 `docs/pitfalls/deployment.md` 坑 #D4（本例是它的产物级同族：验收步骤写了却没进 checklist，就没人对它负责）。
