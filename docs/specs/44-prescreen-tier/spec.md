...
> 最后更新：2026-09-24（**步1 已交付并有生产读数**：`npm test` 621/621、push-CI success、冒烟 20/20、
> 执行锁 E1~E3 真跑过生产模式；生产第一期 `daily_reports.id=68` 实测 `cap=2 → 2000 篇/202 源 → 337 篇/202 源`，
> 覆盖 2.15×、调用量 −32.6%，并把本 spec 两处预估证伪（§八）；**步2 未开工**。前身
> `docs/specs/23-information-overload-defense.md`，本 spec 补的是 23 完全没有的一层：
> **进模型之前的零额度预筛**。判据文档：
...
## 二、实测依据（全部只读探针，零 AI 消耗；逐条读数与取法见 `docs/eval/2026-09-24-prescreen-step1.md`）
...
| 级3 + 保留 500 上限 | ~~覆盖可到 ~450 源，**调用量不变**~~ → **实测：202 源 / 337 次调用（09-24 第一期 id=68，见 §八 与 ISSUES H25）** | 同上 |
...
- 新增 `lib/filter-rules.js`（**步2 待建，本 spec 批准前该文件不存在**）：纯函数 `ruleReject({title, summary, sourceUrl})` → `{hit, why}`。承接三类**零判断**判据：① `prompts/filter.md:18` 的强制压分负例词表（从 prompt 搬进代码）；② commit / changelog 形标题与 `github.com/*/commits/*` 流；③ 纯链接列表薄正文。命中即**不发模型**（这才叫级4 零 token）。 <!-- doc-lint:ignore：拟建文件，见 DOC_GOVERNANCE §2 精炼红线「状态词不许压成路径」-->
...
- ~~**`prescreen.perSourceCap` 没有后台写入口**：`PUT /api/settings` 不含 `prescreen` 段，现在只能直改库里的 settings 行。~~
  → **09-24 已解决**：`handleSettingsPut` 加了 `prescreen.perSourceCap` 分支（1~100 整数校验，坏值 400），
  与消费方读的**点分键**同名同形；起因是用户那句「要能让功能正式可以使用，而不是验证一半、未开发一半」。
  仍没做的是**后台 UI 上的那一格**（早报设置页还没有这个输入框）——键可写 ≠ 可配，剩下的是前端活。
- **历史行的耗时不可比**：`elapsedMin` ÷10 已修，但旧行仍按 1/10 存在同一列里，看趋势前先换算。
- ~~**步1 之后仍会截断（预期，非回归）**：合格数 742 > 上限 500，调用量没变；预算与 gap 按裁定继续暂缓。~~
  → **09-24 第一期实测把这条的两处判断都证伪了**（原文保留，读数见 `docs/eval/2026-09-24-prescreen-step1.md` §八/§九）：
  ① 调用量**不是没变**，是 500 → **337**（−32.6%），因为 202 源 × cap 2 = 404 上限，压根填不满 500 个坑；
  ② `truncated` **确实仍为 true**，但根因不是候选量而是**单篇耗时**——同预算 45min 下本期 9.0 s/篇（id66 是 5.9 s/篇），
  299 次尝试后 38 篇没轮到。结论：预算/gap 继续暂缓可以，但**别再把截断记在候选量头上**。
- **新天花板：`CANDIDATE_POOL_READ=2000` 被吃满**（`prescreen.pool=2000` 恰等于上限，而窗口全量是 2,688 篇 / 469 源）。
  cap 现在是在"被 2000 行截断、只剩 202 个源"的池上做配额 → 覆盖 2.15× 而非预估 4.8×。
  要么抬宽池读，要么把限量下推进 SQL（但那是第四份实现，违 §1.6 架构纪律的"唯一实现"取向）。

## 九、待你拍板（原三项，已定两项；实测后又添一项）

1. ~~`cap` 默认值与来源~~ → 已定：**2，走 settings 键**（用户 09-23 裁定 ≤2）。
2. **`score` 兼容层留不留**（未决，属步2）：保持 `{score,ignore,...}` 契约可护住两条现存锁；
   改成三问形状更干净，但要同步改锁——那把锁的"为什么"就是你那句「初筛是削减不是理解」。
3. ~~步1 与步2 同批还是分两批~~ → 已定：**分两批，先步1**（用户 09-24）。
4. ~~**宽池读 `CANDIDATE_POOL_READ=2000` 抬不抬**~~ → **已定并实施（用户 09-24「抬」）**：2000 → **6000**。
   单一取值写进 `lib/prescreen.js#CANDIDATE_POOL_READ`，runner 与两份 api 同取此数，新增锁 **P9** 钉"四处不分叉"。
   取值依据（只读取数）：24h 全量 **3,645 篇 / 327 源**、30h **4,361 篇 / 387 源**，而轻量列（title+summary+url）合计仅
   **928 KB / 3,645 行** → 过去"宽池不能抬"的顾虑只对 `content_html` 成立，对轻量列不成立；6000 ≈ 1.5× 30h 池，留增长余量。
   预期下一期读数：`prescreen.pool` 不再等于常数（说明真取到全量）、Σmin(n,2)=**530 > 500** → 500 个坑重新填满、
   覆盖往 327 源走、调用量回到 500（**注意：这才是"额度换覆盖"的原意**）。
   → **同日 09:12Z 兑现**（`daily_reports.id=69`，由 `POST /api/daily-generate` 写入）：`pool=3806`（不再顶格）、
   `kept=500`、`keptSources=313` —— 覆盖 202 → **313 源**。读数明细 `docs/eval/2026-09-24-prescreen-step1.md` §十二。
