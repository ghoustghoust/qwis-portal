...
4. **宽池读 `CANDIDATE_POOL_READ=2000` 抬不抬**（09-24 第一期实测新增）：现在它是覆盖率的真天花板
   （202 源 vs 窗口全量 469 源），500 个坑还空着 163 个。抬到窗口全量（≈2,688 轻量行）只需改一个常量，
   代价是多拉 ~690 行轻量列；不抬则级3 的覆盖率永远由"读多少行"决定，而不是由"每源几条"决定。
