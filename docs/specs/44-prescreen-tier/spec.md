...
5. **统计臂（NB / 互补 NB / TF-IDF）要不要进步2 的形态：已实测出安全工作点，等你点头定形态**（用户 09-24「综合考虑是否采用」）：
   用**行为标签**（正＝已读/稍后读/精选，负＝保留策略判定可删）做三臂回测，训练侧完全不含"你读过的那批"→
   AUC **.66~.71**（95% 自助区间与"长度单特征"对照 .43~.59 **不重叠**，也与随机分开）；
   但把约束放在**误砍 ≤3%** 时，**最多只能砍池子的 10%**（≤5% 时 15%）；砍到 50% 会连带干掉你读过的 **26~28%**。
   ⇒ 本 spec 的取数口径随之改写：**统计臂不是"整段替代 AI 初筛"的那一层**（省不到 67~98min），
   能当的是**形态规则之后的兜底排序**（池子超配额时优先牺牲最低分的底部 10%）。三臂之间统计上不可分，"选哪个模型"是伪问题。
   读数与边界：`docs/eval/2026-09-24-prescreen-labels.md`「三臂行为回测」；命令 `node tools/eval-prescreen-behavior.cjs`。
