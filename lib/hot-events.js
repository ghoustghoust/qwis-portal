...
const EVENTS_WINDOW_H = 168; // 采样窗口：近 7 天（事件最后一篇报道满 7 天即下榜）
...
function aggregateEventRows(rows, { nowMs = Date.now(), halfLifeH = EVENTS_HALF_LIFE_H, simThreshold = EVENTS_SIM_THRESHOLD } = {}) {
  ...
  const solo = items
    .filter((i) => !clusteredIds.has(i.id) && i.source_type !== 'hotlist' && (Number(i.score) || 0) >= 55) // 评分覆盖初期降到 55，随积累提升可回 60
...
// 采样 SQL（两端共用）：全源窗口函数每源≤10 条 + 摘要/译文 SQL 侧截断
