...
// 分组口径（2026-09-14 收紧）：仅「人工智能」系与纯「AI」组——LIKE '%AI%' 曾把「AI·前沿」组里的
//   Reddit·Linux/DevOps 混合订阅源整组放行（DevOps 求助帖混进精选，用户验收发现）。
// LIKE（大小写不敏感）只放无歧义长词；短缩写走 GLOB（大小写敏感）防误伤——RAG 不匹配 garage；
// 裸「AI」走词边界 GLOB，防匹配 SAID/CHAIR/EMAIL 等。
const AI_GROUP_COND = "(g.name LIKE '%人工智能%' OR UPPER(COALESCE(g.name,'')) = 'AI')";
const AI_KW_LIKE = [
  // 中文主题词
