...
/** 取某张表的原始 CREATE 语句（给需要"与建表源同形状"的工具复用，不再复制 DDL） */
function createSqlOf(root, rel, table) {
  const p = path.join(root, rel);
  if (!fs.existsSync(p)) return null;
  const chunks = scan(fs.readFileSync(p, 'utf8')).strings.map((s) => s.s);
  for (const sql of chunks) {
    for (const m of sql.matchAll(CREATE_RE)) {
      if (m[1] === table) return m[0].replace(/\s*$/, '');
    }
  }
  return null;
}

module.exports = { findSchemaGaps, parseDbFile, createSqlOf };
