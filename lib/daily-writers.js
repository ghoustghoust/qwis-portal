...
//   setBad      —— SQL 里给档位赋值时只放行 `CAST(? AS INTEGER)`：
//                   带引号字面量 `'1'` 落成 JSON 文本（收口前那 1 行就是它），
//                   **绑一个裸 JS number 也不对** —— 驱动送的是 double，json_set 存成 `1.0`
//                   （real，坑 #70 实测）。两种都判红，只有 CAST 过。
const SCHEMA_CARRIED = /DAILY_SCHEMA_VERSION\s*\.\s*(KEYWORD|AI)/;
const SCHEMA_BARE = /schemaVersion\s*:\s*\d/;
// 档位赋值的"取到值再看形状"判据（不用 `,\s*(?!CAST)` 这种 lookahead：
// `\s*` 会回溯出"零个空格"的匹配，`(?!CAST)` 在空格位置上成立 → CAST 的正确写法反被误判，实测踩到）。
const SCHEMA_SET_VALUE = /'\$\.schemaVersion'\s*,\s*([^)]*)\)/;
function schemaSetIsBad(sql) {
  const m = SCHEMA_SET_VALUE.exec(String(sql || ''));
  if (!m) return false;
  return !/^\s*CAST\s*\(/i.test(m[1]);
}
      ...
      for (const c of chunks) {
        ...
        found.push({
          ...
          schema: dynamic ? null : {
            ...
            stringTyped: schemaSetIsBad(c.sql),
          },
        });
      }
    }
...
//   `json_set(stats, '$.schemaVersion', '1')` —— 写成 JSON 字符串；
// 而那条 UPDATE 不在任何 INSERT 的宿主函数里（它在调用方做善后），所以光靠写入点清单看不见它 →
// 单独扫一遍所有语句字面量，形状判据复用上面那条 SCHEMA_SET_BAD（只放行 CAST）。
...
module.exports = { findDailyReportWriters, findStatsSchemaTypeViolations, schemaSetIsBad, sqlChunks, INSERT_MARK, INSERT_DYNAMIC, GATE_USE, EXEC, SCHEMA_CARRIED, SCHEMA_BARE };