...
CREATE TABLE IF NOT EXISTS sources (
  ...
  reader_visible INTEGER DEFAULT 1,
...
async function ensureSchema() {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      // 27b 四轴一次性迁移：focus=1 → spotlight=1 + subscription.ids 初始化
      // ⚠️ 必须用裸 dbGet/dbRun——getSetting/setSetting 内部会 await ensureSchema()，
      // 在此调用 = 自等待死锁（schemaReady 等待自己）
      try {
        const raw = async (key) => {
        ...
        const put = async (key, val) => dbRun(
          'INSERT INTO settings(key, value) VALUES(?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value',
          key, JSON.stringify(val)
        );
        if (!(await raw('axes.migrated'))) {
          await dbRun('UPDATE sources SET spotlight=1 WHERE COALESCE(focus,0)=1 AND COALESCE(spotlight,0)=0');
          if (!Array.isArray(await raw('subscription.ids'))) {
            const rows = await dbAll('SELECT id FROM sources WHERE COALESCE(spotlight,0)=1');
            await put('subscription.ids', rows.map((r) => r.id));
          }
          await put('axes.migrated', 1);
        }
      } catch { /* 迁移失败不阻断 schema 就绪 */ }
    })();
  }
  return schemaReady;
}
