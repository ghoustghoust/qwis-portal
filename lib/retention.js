...
//
// 还有一条不许顺手改的语义（保守优先）：云端今天的判据是 `read_at IS NULL`。线上另有 2.5 万行
// `read_at` 是**字面字符串 `'null'`**（B15/BL10：`tools/migrate-to-turso.js` 把 NULL 序列化成字符串），
// 语义上它们等于"未读"，但**在数据订正与内容级备份落地前不许把它们变成可删**（B101 与放行表 #8
// 明写：清 `read_at` 会让这批重新变成可被删，而此刻云端没有内容级恢复路径 = B103）。
// 所以 `UNREAD_COND` 保持 `read_at IS NULL`，把 `'null'`/`''` 暂时当"非未读"，订正后再翻（改这里 = 改语义，必须同批动 #8 的锁）。
'use strict';
