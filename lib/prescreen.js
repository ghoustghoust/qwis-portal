...
// 配额值的唯一归一：缺键/空串/布尔/小数/越界一律回默认并允许调用方出声。
// 口径照 `lib/brief-guards.js#dailyMinScoreOf`：`Number('')` 是 0，而 0 的意思是"配额关了"——
// 配置缺失绝不能读成"故意设为 0"（坑 #38「静默降级=没人知道」同族）。
function prescreenCapOf(raw) {
  if (raw === null || raw === undefined || raw === '' || typeof raw === 'boolean') return DEFAULT_PER_SOURCE_CAP;
  const n = Number(raw);
  return Number.isInteger(n) && n >= 1 && n <= 100 ? n : DEFAULT_PER_SOURCE_CAP;
}
...
module.exports = { DEFAULT_PER_SOURCE_CAP, DEFAULT_POOL_LIMIT, prescreenCapOf, applySourceQuota, prescreenStats };
